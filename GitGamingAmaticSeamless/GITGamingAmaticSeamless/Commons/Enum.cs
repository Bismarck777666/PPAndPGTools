...
namespace GITIGamingWebsite.Commons
{
    ...
    {
        private static CurrencyInfo _sInstance = new CurrencyInfo();
        public static CurrencyInfo Instance     => _sInstance;

        public Dictionary<Currencies, CurrencyObj> _currencyInfo;
        private CurrencyInfo()
        {
            _currencyInfo = new Dictionary<Currencies, CurrencyObj>()
            {
        ...
        {
            foreach(KeyValuePair<Currencies, CurrencyObj> pair in _currencyInfo)
            {
                if (pair.Key == currency)
                    return pair.Value.CurrencySymbol;
            }
            return "";
        }

        public string getCurrencyText(Currencies currency)
        {
            foreach (KeyValuePair<Currencies, CurrencyObj> pair in _currencyInfo)
            {
                if (pair.Key == currency)
                    return pair.Value.CurrencyText;
            }
            return "";
        }

        public double getCurrencyRate(Currencies currency)
        {
            foreach (KeyValuePair<Currencies, CurrencyObj> pair in _currencyInfo)
            {
                if (pair.Key == currency)
        ...
        public Currencies getCurrencyFromText(string currencyText)
        {
            foreach(KeyValuePair<Currencies, CurrencyObj> pair in _currencyInfo)
            {
                if (pair.Value.CurrencyText == currencyText)
                {
                    return pair.Key;
                }
            }
