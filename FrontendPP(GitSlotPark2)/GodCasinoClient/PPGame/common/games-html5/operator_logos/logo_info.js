...
UHTPatch({
	...
	apply: function()
	{
		...
		MultiLobbyConnection.prototype.ParseVendors = function(dict)
		{
			...
			if(MultiLobbyConnection.prototype.ParseEvents != undefined)
			{
				for (var i = 0; i < list.length; ++i)
					this.ParseEvents(list[i]);
			}
					...
					"configs": [
					  {
						...
						"hideWinLimitPaytable" : true,
						"customText" : {
							en:`The max prize in this game is 20,000x and can be achieved during the base game and and the buy "DUEL" option`,
...
UHTPatch({
	...
	apply:function()
	{
		...
		if ((minSpinBetCents == false) || (["vswaysmahwblck"].indexOf(window["UHT_GAME_CONFIG"]["GAME_SYMBOL"]) != -1))
			return;
