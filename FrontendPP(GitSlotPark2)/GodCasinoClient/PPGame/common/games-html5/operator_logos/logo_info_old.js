	...
	apply: function()
	{
		...
		var OnXTGameInit = function()
		{
			...
			var warningTextMap = {
				...
				ar:`Ø§Ù„Ø±Ù‡Ø§Ù† Ø§Ù„Ø­Ø§Ù„ÙŠ Ø£Ø¹Ù„Ù‰ Ù…Ù† Ø§Ù„Ø­Ø¯ Ø§Ù„Ø£Ø¯Ù†Ù‰ ${minBetValue}`,
				bg:`Ð¢ÐµÐºÑƒÑ‰Ð¸ÑÑ‚ Ð·Ð°Ð»Ð¾Ð³ Ðµ Ð¿Ð¾-Ð²Ð¸ÑÐ¾Ðº Ð¾Ñ‚ Ð¼Ð¸Ð½Ð¸Ð¼ÑƒÐ¼Ð° Ð½Ð° ${minBetValue}`,
				cs:`AktuÃ¡lnÃ­ sÃ¡zka je vyÅ¡Å¡Ã­ neÅ¾ minimum ${minBetValue}`,
				da:`NuvÃ¦rende indsats er hÃ¸jere end minimum af ${minBetValue}`,
				de:`Der aktuelle Einsatz ist hÃ¶her als das Minimum von ${minBetValue}`,
				el:`Î¤Î¿ Ï„ÏÎ­Ï‡Î¿Î½ Ï€Î¿Î½Ï„Î¬ÏÎ¹ÏƒÎ¼Î± ÎµÎ¯Î½Î±Î¹ Ï…ÏˆÎ·Î»ÏŒÏ„ÎµÏÎ¿ Î±Ï€ÏŒ Ï„Î¿ ÎµÎ»Î¬Ï‡Î¹ÏƒÏ„Î¿ Ï„Î¿Ï… ${minBetValue}`,
				es:`La apuesta actual es superior al mÃ­nimo de ${minBetValue}`,
				et:`Praegune panus on suurem kui miinimum ${minBetValue}`,
				fa:`Ø´Ø±Ø· ÙØ¹Ù„ÛŒ Ø¨ÛŒØ´ØªØ± Ø§Ø² Ø­Ø¯Ø§Ù‚Ù„ ${minBetValue} Ø§Ø³Øª`,
				fi:`Nykyinen panos on suurempi kuin ${minBetValue}:n vÃ¤himmÃ¤ismÃ¤Ã¤rÃ¤`,
				fr:`La mise actuelle est supÃ©rieure au minimum de ${minBetValue}`,
				hr:`Trenutna oklada je veÄ‡a od minimalne ${minBetValue}`,
				hu:`Az aktuÃ¡lis tÃ©t nagyobb mint a ${minBetValue} minimÃ¡lis Ã©rtÃ©ke`,
				hy:`Ô¸Õ¶Õ©Õ¡ÖÕ«Õ¯ Õ­Õ¡Õ²Õ¡Õ¤Ö€Õ¸Ö‚ÕµÖ„Õ¨ Õ¢Õ¡Ö€Õ±Ö€ Õ§ Õ¶Õ¾Õ¡Õ¦Õ¡Õ£Õ¸Ö‚ÕµÕ¶Õ«Ö ${minBetValue}`,
				id:`Taruhan saat ini lebih tinggi dari minimum ${minBetValue}`,
				it:`La puntata attuale Ã¨ superiore al minimo di ${minBetValue}`,
				ja:`ç¾åœ¨ã®ãƒ™ãƒƒãƒˆãŒ${minBetValue}ã®æœ€å°å€¤ã‚ˆã‚Šé«˜ã„`,
				ka:`áƒ›áƒ˜áƒ›áƒ“áƒ˜áƒœáƒáƒ áƒ” áƒ¤áƒ¡áƒáƒœáƒ˜ áƒ›áƒ˜áƒœáƒ˜áƒ›áƒ£áƒ›áƒ–áƒ” áƒ›áƒ”áƒ¢áƒ˜áƒ ${minBetValue}`,
				ko:`í˜„ìž¬ ë² íŒ…ì´ ìµœì†Œê°’ ${minBetValue}ë³´ë‹¤ ë†’ìŠµë‹ˆë‹¤.`,
				lt:`Dabartinis statymas yra didesnis uÅ¾ ${minBetValue} minimumÄ…`,
				lv:`PaÅ¡reizÄ“jÄ likme ir lielÄka par ${minBetValue} minimumu`,
				ms:`Pertaruhan semasa lebih tinggi daripada minimum ${minBetValue}`,
				nl:`Huidige inzet is hoger dan het minimum van ${minBetValue}`,
				no:`Gjeldende innsats er hÃ¸yere enn minimum av ${minBetValue}`,
				pl:`Aktualny zakÅ‚ad jest wyÅ¼szy niÅ¼ minimum ${minBetValue}`,
				pt:`A aposta atual Ã© superior ao mÃ­nimo de ${minBetValue}`,
				ro:`Miza curenta e mai mare decat minimul de ${minBetValue}`,
				ru:`Ð¢ÐµÐºÑƒÑ‰Ð°Ñ ÑÑ‚Ð°Ð²ÐºÐ° Ð²Ñ‹ÑˆÐµ Ð¼Ð¸Ð½Ð¸Ð¼Ð°Ð»ÑŒÐ½Ð¾Ð¹ ${minBetValue}`,
				sk:`AktuÃ¡lna stÃ¡vka je vyÅ¡Å¡ia ako minimum ${minBetValue}`,
				sr:`Ð¢Ñ€ÐµÐ½ÑƒÑ‚Ð½Ð° Ð¾Ð¿ÐºÐ»Ð°Ð´Ð° Ñ˜Ðµ Ð²ÐµÑ›Ð° Ð¾Ð´ Ð¼Ð¸Ð½Ð¸Ð¼Ð°Ð»Ð½Ðµ ${minBetValue}`,
				sv:`Aktuell insats Ã¤r hÃ¶gre Ã¤n minimum av ${minBetValue}`,
				th:`à¸à¸²à¸£à¹€à¸”à¸´à¸¡à¸žà¸±à¸™à¸›à¸±à¸ˆà¸ˆà¸¸à¸šà¸±à¸™à¸ªà¸¹à¸‡à¸à¸§à¹ˆà¸²à¸‚à¸±à¹‰à¸™à¸•à¹ˆà¸³ ${minBetValue}`,
				tr:`Mevcut bahis minimum ${minBetValue} deÄŸerinden yÃ¼ksektir`,
				uk:`ÐŸÐ¾Ñ‚Ð¾Ñ‡Ð½Ð° ÑÑ‚Ð°Ð²ÐºÐ° Ð±Ñ–Ð»ÑŒÑˆÐ° Ð·Ð° Ð¼Ñ–Ð½Ñ–Ð¼Ð°Ð»ÑŒÐ½Ñƒ ${minBetValue}`,
				vi:`CÆ°á»£c hiá»‡n táº¡i cao hÆ¡n má»©c tá»‘i thiá»ƒu ${minBetValue}`,
				zh:`å½“å‰æŠ•æ³¨é¢é«˜äºŽ ${minBetValue} çš„æœ€å°å€¼`,
				zt:`ç›®å‰çš„æŠ•æ³¨é«˜æ–¼ ${minBetValue} çš„æœ€å°å€¼`
			};
	...
	apply: function()
	{
		if (IsRequired("SHOXC_LIM100"))
		{
			var textDict = {
				...
				ar:["Ø§Ù„Ø¬Ø§Ø¦Ø²Ø© Ø§Ù„Ù‚ØµÙˆÙ‰ ÙÙŠ Ù‡Ø°Ù‡ Ø§Ù„Ù„Ø¹Ø¨Ø© Ù‡ÙŠ {0}","Ø§Ù„Ø¬Ø§Ø¦Ø²Ø© Ø§Ù„Ù‚ØµÙˆÙ‰ ÙÙŠ Ù‡Ø°Ù‡ Ø§Ù„Ù„Ø¹Ø¨Ø© Ù‡ÙŠ {0} Ù…Ø¹ ÙØ±ØµØ© Ù„ØªØ³Ø¬ÙŠÙ„ 1 ÙÙŠ {1}"],
				bg:["ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»Ð½Ð°Ñ‚Ð° Ð¿ÐµÑ‡Ð°Ð»Ð±Ð° Ð² Ñ‚Ð°Ð·Ð¸ Ð¸Ð³Ñ€Ð° Ðµ {0}","ÐœÐ°ÐºÑÐ¸Ð¼Ð°Ð»Ð½Ð°Ñ‚Ð° Ð¿ÐµÑ‡Ð°Ð»Ð±Ð° Ð² Ñ‚Ð°Ð·Ð¸ Ð¸Ð³Ñ€Ð° Ðµ {0}, Ð° Ð²ÐµÑ€Ð¾ÑÑ‚Ð½Ð¾ÑÑ‚Ñ‚Ð° Ð´Ð° Ð±ÑŠÐ´ ÑÐ¿ÐµÑ‡ÐµÐ»ÐµÐ½Ð° Ðµ {1}"],
				cs:["MaximÃ¡lnÃ­ vÃ½hra v tÃ©to hÅ™e je {0}","MaximÃ¡lnÃ­ vÃ½hra v tÃ©to hÅ™e je {0} s Å¡ancÃ­ na vÃ½hru 1 ku {1}"],
				da:["Maks. prÃ¦mie i dette spil er {0}","Maks. prÃ¦mie i dette spil er {0} med en chance for at ramme pÃ¥ 1 ud af {1}"],
				de:["Der max. Preis in diesem Spiel betrÃ¤gt {0}","Der max. Preis in diesem Spiel betrÃ¤gt {0}, mit einer Chance auf 1 in {1}"],
				el:["Î¤Î¿ Î¼Î­Î³Î¹ÏƒÏ„Î¿ Î­Ï€Î±Î¸Î»Î¿ ÏƒÎµ Î±Ï…Ï„ÏŒ Ï„Î¿ Ï€Î±Î¹Ï‡Î½Î¯Î´Î¹ ÎµÎ¯Î½Î±Î¹ {0}","Î¤Î¿ Î¼Î­Î³Î¹ÏƒÏ„Î¿ Î­Ï€Î±Î¸Î»Î¿ ÏƒÎµ Î±Ï…Ï„ÏŒ Ï„Î¿ Ï€Î±Î¹Ï‡Î½Î¯Î´Î¹ ÎµÎ¯Î½Î±Î¹ {0} Î¼Îµ Ï€Î¹Î¸Î±Î½ÏŒÏ„Î·Ï„Î± Î½Î± Ï€ÎµÏ„ÏÏ‡ÎµÏ„Îµ 1 ÏƒÎµ {1}"],
				es:["El premio mÃ¡x de este juego es {0}","El premio mÃ¡x de este juego es {0} con una posibilidad de conseguirlo de 1 sobre {1}"],
				et:["Selle mÃ¤ngu maksimumauhind on {0}","Selle mÃ¤ngu maksimumauhind on {0} ja maksimumauhinna vÃµitmise tÃµenÃ¤osus on 1:{1}"],
				fa:["Ø­Ø¯Ø§Ú©Ø«Ø± Ø¬Ø§ÛŒØ²Ù‡ Ø¯Ø± Ø§ÛŒÙ† Ø¨Ø§Ø²ÛŒ {0} Ø§Ø³Øª","Ø­Ø¯Ø§Ú©Ø«Ø± Ø¬Ø§ÛŒØ²Ù‡ Ø¯Ø± Ø§ÛŒÙ† Ø¨Ø§Ø²ÛŒ {0} Ø¨Ø§ Ø´Ø§Ù†Ø³ Ù†Ù…Ø§ÛŒØ§Ù† Ø´Ø¯Ù† 1 Ø¯Ø± {1} Ø§Ø³Øª."],
				fi:["TÃ¤mÃ¤n pelin enimmÃ¤ispalkinto on {0}","TÃ¤mÃ¤n pelin enimmÃ¤ispalkinto on {0}, ja osuman todennÃ¤kÃ¶isyys on 1:{1}"],
				fr:["Le prix max dans ce jeu est de{0}","Le prix max dans ce jeu est de {0} avec des chances d'Ãªtre obtenu de 1 sur {1}"],
				hr:["Maks. nagrada u ovoj igri iznosi {0}","Maks. nagrada u ovoj igri iznosi {0} s moguÄ‡nosti da dobijete 1 u {1}"],
				hu:["Ebben a jÃ¡tÃ©kban a max. nyeremÃ©ny {0}","Ebben a jÃ¡tÃ©kban max. nyeremÃ©ny {0}, a kipÃ¶rgetÃ©s esÃ©lye 1 az {1}-hez"],
				hy:["Ô±ÕµÕ½ Õ­Õ¡Õ²Õ¸Ö‚Õ´ Õ¡Õ¼Õ¡Õ¾Õ¥Õ¬Õ¡Õ£Õ¸Ö‚ÕµÕ¶ Õ´Ö€ÖÕ¡Õ¶Õ¡Õ¯Õ¶ Õ§ {0}","Ô±ÕµÕ½ Õ­Õ¡Õ²Õ¸Ö‚Õ´ Õ¡Õ¼Õ¡Õ¾Õ¥Õ¬Õ¡Õ£Õ¸Ö‚ÕµÕ¶ Õ´Ö€ÖÕ¡Õ¶Õ¡Õ¯Õ¶ Õ§{0} Õ {1}-Õ«Ö 1-Õ¶ Õ¨Õ¶Õ¯Õ¶Õ¥Õ¬Õ¸Ö‚ Õ°Õ¡Õ¾Õ¡Õ¶Õ¡Õ¯Õ¡Õ¶Õ¸Ö‚Õ©ÕµÕ¡Õ´Õ¢Ö‰"],
				id:["hadiah maksimum di permainan ini adalah {0}","Hadiah maksimum di permainan ini adalah {0} dengan peluang meraih 1 di {1}"],
				it:["Il premio massimo in questo gioco Ã¨ pari a {0}","Il premio massimo in questo gioco Ã¨ pari a {0} con una possibilitÃ  di ottenere 1 in {1}"],
				ja:["ã“ã®ã‚²ãƒ¼ãƒ ã®æœ€é«˜è³žé‡‘ã¯{0}ã§ã™ã€‚","ã“ã®ã‚²ãƒ¼ãƒ ã®æœ€é«˜è³žé‡‘ã¯{0}ã§ã€å½“ãŸã‚‹ç¢ºçŽ‡ã¯{1}åˆ†ã®1ã§ã™"],
				ka:["áƒ›áƒáƒ¥áƒ¡áƒ˜áƒ›áƒáƒšáƒ£áƒ áƒ˜ áƒžáƒ áƒ˜áƒ–áƒ˜ áƒáƒ› áƒ—áƒáƒ›áƒáƒ¨áƒ¨áƒ˜ áƒ¨áƒ”áƒáƒ“áƒ’áƒ”áƒœáƒ¡ {0}-áƒ¡","áƒ›áƒáƒ¥áƒ¡áƒ˜áƒ›áƒáƒšáƒ£áƒ áƒ˜ áƒžáƒ áƒ˜áƒ–áƒ˜ áƒáƒ› áƒ—áƒáƒ›áƒáƒ¨áƒ¨áƒ˜ áƒ¨áƒ”áƒáƒ“áƒ’áƒ”áƒœáƒ¡ {0}-áƒ¡, áƒ›áƒáƒ’áƒ”áƒ‘áƒ˜áƒ¡ áƒáƒšáƒ‘áƒáƒ—áƒáƒ‘áƒ â€” {1}"],
				ko:["ê²Œìž„ ìµœëŒ€ ìƒê¸ˆì€ {0}","ê²Œìž„ ìµœëŒ€ ìƒê¸ˆì€ {0}, {1}ì—ì„œ 1 ížˆíŠ¸ ê°€ëŠ¥"],
				lt:["Maksimalus Å¡io Å¾aidimo prizas yra {0}","Maksimalus Å¡io Å¾aidimo prizas yra {0}, o tikimybÄ— iÅ¡sukti yra 1 iÅ¡ {1}"],
				lv:["Maks. balva Å¡ajÄ spÄ“lÄ“ ir {0}","Maks. balva Å¡ajÄ spÄ“lÄ“ ir {0} ar iespÄ“ju uzgriezt 1 no {1}"],
				...
				no:["Maks gevinsten i dette spillet er {0}","Maks gevinsten i dette spillet er {0} med en 1 i {1} sjanse til Ã¥ treffe"],
				pl:["Maksymalna nagroda w tej grze wynosi {0}","Maksymalna nagroda w tej grze wynosi {0} z szansÄ… wygranej 1 na {1}"],
				pt:["O prÃªmio mÃ¡ximo neste jogo Ã© {0}","O prÃªmio mÃ¡ximo neste jogo Ã© {0}, com uma possibilidade de ser obtido de 1 em {1}"],
				ro:["The max prize in this game is {0}","The max prize in this game is {0} with a chance to hit of 1 in {1}"],
				ru:["ÐœÐ°ÐºÑ. Ð¿Ñ€Ð¸Ð· Ð² ÑÑ‚Ð¾Ð¹ Ð¸Ð³Ñ€Ðµ ÑÐ¾ÑÑ‚Ð°Ð²Ð»ÑÐµÑ‚ {0}","ÐœÐ°ÐºÑ. Ð¿Ñ€Ð¸Ð· Ð² ÑÑ‚Ð¾Ð¹ Ð¸Ð³Ñ€Ðµ ÑÐ¾ÑÑ‚Ð°Ð²Ð»ÑÐµÑ‚ {0} Ñ ÑˆÐ°Ð½ÑÐ¾Ð¼ Ð²Ñ‹Ð¿Ð°Ð´ÐµÐ½Ð¸Ñ 1 Ð² {1}"],
				sk:["MaximÃ¡lna cena v thejto hre je {0}","MaximÃ¡lna cena v tejto hre je {0} so Å¡ancou na vÃ½hru 1 ku {1}"],
				sr:["Maks. nagrada u ovoj igri je {0}","Maks. nagrada u ovoj igri je {0} sa Å¡ansom za dobijanje od 1 prema {1}"],
				sv:["Maxpriset i det hÃ¤r spelet Ã¤r {0}","Maxpriset i det hÃ¤r spelet Ã¤r {0} med en chans att trÃ¤ffa 1 pÃ¥ {1}"],
				th:["à¸£à¸²à¸‡à¸§à¸±à¸¥à¸ªà¸¹à¸‡à¸ªà¸¸à¸”à¹ƒà¸™à¹€à¸à¸¡à¸™à¸µà¹‰à¸„à¸·à¸­ {0}","à¸£à¸²à¸‡à¸§à¸±à¸¥à¸ªà¸¹à¸‡à¸ªà¸¸à¸”à¹ƒà¸™à¹€à¸à¸¡à¸™à¸µà¹‰à¸„à¸·à¸­ {0} à¹‚à¸”à¸¢à¸¡à¸µà¹‚à¸­à¸à¸²à¸ªà¸—à¸µà¹ˆà¸ˆà¸°à¸›à¸£à¸²à¸à¸ 1 à¹ƒà¸™ {1}"],
				tr:["Bu oyundaki maks Ã¶dÃ¼l {0}'tir","Bu oyundaki maks Ã¶dÃ¼l {0}'tir ve {1}'de 1 denk gelme ihtimali vardÄ±r"],
				uk:["ÐœÐ°ÐºÑ. Ð¿Ñ€Ð¸Ð· Ñƒ Ñ†Ñ–Ð¹ Ð³Ñ€Ñ– ÑÑ‚Ð°Ð½Ð¾Ð²Ð¸Ñ‚ÑŒ {0}","ÐœÐ°ÐºÑ. Ð¿Ñ€Ð¸Ð· Ñƒ Ñ†Ñ–Ð¹ Ð³Ñ€Ñ– ÑÑ‚Ð°Ð½Ð¾Ð²Ð¸Ñ‚ÑŒ {0} Ð· ÑˆÐ°Ð½ÑÐ¾Ð¼ Ð²Ð¸Ð¿Ð°Ð´Ð°Ð½Ð½Ñ 1 Ð² {1}"],
				vi:["Giáº£i thÆ°á»Ÿng tá»‘i Ä‘a trong trÃ² chÆ¡i nÃ y lÃ  {0}","Giáº£i thÆ°á»Ÿng tá»‘i Ä‘a trong trÃ² chÆ¡i nÃ y lÃ  {0} vá»›i cÆ¡ há»™i giÃ nh Ä‘Æ°á»£c lÃ  1 trong {1}"],
				zh:["æœ¬æ¸¸æˆä¸­çš„æœ€å¤§å¥–åŠ±ä¸º{0}","æœ¬æ¸¸æˆä¸­çš„æœ€å¤§å¥–åŠ±ä¸º{0}ï¼Œå¹¶æœ‰æœºä¼šåœ¨{1}ä¸­å‡ºçŽ°1"],
				zt:["æœ¬éŠæˆ²ä¸­çš„æœ€å¤§çŽå‹µç‚º{0}","æœ¬éŠæˆ²ä¸­çš„æœ€å¤§çŽå‹µç‚º{0}ï¼Œä¸¦æœ‰æ©Ÿæœƒåœ¨{1}ä¸­å‡ºç¾1"]
			};
				  ...
				  "configs": [
					{
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 7.300x, con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 7.300x com uma taxa de acerto de 1 em 1.000.000.000 e pode ser obtido no jogo base`
					  }
					},
					{
					  "rtp": "96.51",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 6.600x, con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 6.600x com uma taxa de acerto de 1 em 1.000.000.000 e pode ser obtido no jogo base`
					  }
					},
					{
					  "rtp": "96.6",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 5.900x, con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 5.900x com uma taxa de acerto de 1 em 1.000.000.000 e pode ser obtido no jogo base`
					  }
					}
				  ]
				},
				  ...
				  "configs": [
					{
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 3.400x, con una tasa de apariciÃ³n de 1 en 1.000.000.000 y puede alcanzarse al elegir la opciÃ³n de 6 tiradas gratis`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 3.400x com uma taxa de acerto de 1 em 1.000.000.000 e pode ser obtido ao escolher o modo de 6 rodadas grÃ¡tis`
					  }
					},
					{
					  "rtp": "96.5",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 4.100x, con una tasa de apariciÃ³n de 1 en 1.000.000.000 y puede alcanzarse al elegir la opciÃ³n de 20 tiradas gratis`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 4.100x com uma taxa de acerto de 1 em 1.000.000.000 e pode ser obtido ao escolher o modo de 20 rodadas grÃ¡tis`
					  }
					},
				  ]
				},
				  ...
				  "configs": [
					{
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 20.000x, con una tasa de apariciÃ³n de 1 en 166.666.667 y puede conseguirse al elegir la opciÃ³n de 5 tiradas gratis con multiplicador inicial de 10x`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 20.000x com uma taxa de acerto de 1 em 1.666.666.667 e pode ser obtido ao escolher o modo de 5 rodadas grÃ¡tis com opÃ§Ã£o multiplicadora inicial de 10x`
					  }
					},
					{
					  "rtp": "95.59",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 20.000x, con una tasa de apariciÃ³n de 1 en 250.000.000 y puede conseguirse al elegir la opciÃ³n de 5 tiradas gratis con multiplicador inicial de 10x`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 20.000x com uma taxa de acerto de 1 em 250.000.000 e pode ser obtido ao escolher o modo de 5 rodadas grÃ¡tis com opÃ§Ã£o multiplicadora inicial de 10x`
					  }
					},
					{
					  "rtp": "96.58",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 20.000x, con una tasa de apariciÃ³n de 1 en 200.000.000 y puede conseguirse al elegir la opciÃ³n de 5 tiradas gratis con multiplicador inicial de 10x`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 20.000x com uma taxa de acerto de 1 em 200.000.000 e pode ser obtido ao escolher o modo de 5 rodadas grÃ¡tis com opÃ§Ã£o multiplicadora inicial de 10x`
					  }
					}
				  ]
				},
				  ...
				  "configs": [
					{
					  ...
					  "customText" : {
						en:`The max prize in this game is 20,000x, with a hitrate of 1 in 166,666,667 and can be achieved when choosing the 5 free spins with 10x starting multiplier option`,
						es:`El premio mÃ¡x de este juego es de 20.000x, con una tasa de apariciÃ³n de 1 en 166.666.667 y puede conseguirse al elegir la opciÃ³n de 5 tiradas gratis con multiplicador inicial de 10x`,
						pt:`O prÃªmio mÃ¡ximo nesse jogo Ã© de 20.000x com uma taxa de acerto de 1 em 166.666.667 e pode ser obtido ao escolher o modo de 5 rodadas grÃ¡tis com opÃ§Ã£o multiplicadora inicial de 10x`
					  }
					},
					{
					  "rtp": "95.59",
				  ...
				  "configs": [
					{
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 12.000x, con una tasa de apariciÃ³n de 1 en 500.000.000 y puede conseguirse al elegir la opciÃ³n tiradas gratis de wilds lluviosos`,
						pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 12.000x, com uma taxa de acerto de 1 em 500.000.000 e pode ser ganho ao escolher o modo "WILDS CHUVOSOS" de rodadas grÃ¡tis`,
					  }
					},
					{
					  "rtp": "95.53",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 12.000x, con una tasa de apariciÃ³n de 1 en 333.333.333 y puede conseguirse al elegir la opciÃ³n tiradas gratis de wilds pegajosos`,
						pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 12.000x, com uma taxa de acerto de 1 em 333.333.333 e pode ser ganho ao escolher o modo "WILDS PEGAJOSOS" de rodadas grÃ¡tis`,
					  }
					},
					{
					  "rtp": "96.55",
					  ...
					  "customText" : {
						...
						es:`El premio mÃ¡x de este juego es de 12.000x, con una tasa de apariciÃ³n de 1 en 200.000.000 y puede conseguirse al elegir la opciÃ³n tiradas gratis de wilds lluviosos`,
						pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 12.000x, com uma taxa de acerto de 1 em 200.000.000 e pode ser ganho ao escolher o modo "WILDS CHUVOSOS" de rodadas grÃ¡tis`,
					  }
					}
				  ]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡ximo de este juego es de 5.000x y puede alcanzarse al elegir la opciÃ³n de tiradas gratis con WILDS EXPANSIVOS`,
							pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 5,000X e pode ser ganho ao escolher o modo "EXPANDING WILD" de jogadas grÃ¡tis`
						  }
					  },
					  {
						"rtp": "95.03",
					...
					"configs": [
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 3.300x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base',
								},
								{
									en:'The max prize in this game is 2,800x with a hitrate of 1 in 1,000,000,000 and can be achieved on the base game',
									es:'El premio mÃ¡x de este juego es de 2.800x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base',
								}
							];
					  ...
					  },
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.300x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
						}
					  },
					  {
						"rtp": "96.00",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 3.400x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
						}
					  }
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es 25.000X`,
							pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 25.000X`
						}
					  },
					  {
						"rtp": "95.52",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.500x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 4.500x e pode ser alcanÃ§ado no jogo base`
						}
					  },
					  {
						"rtp": "94.24",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 125.000.000 y se puede conseguir en el juego base`,
						}
					  },
					  {
						"rtp": "95.54",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 142.857.143 y se puede conseguir en el juego base`,
						}
					  },
					  {
						"rtp": "96.55",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 111.111.111 y se puede conseguir en el juego base`,
						}
					  }
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 20.000x y se puede conseguir en el juego base y en la funciÃ³n DUEL`,
							pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 20.000X e pode ser ganho no jogo base e no modo "DUEL"`
						}
					  },
					  {
						"rtp": "95.00",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 3.620x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.78",
						...
						"customText" : {
							...
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 3.670x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.77",
						...
						"customText" : {
							...
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 3.590x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  }
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.888x con una tasa de apariciÃ³n de 1 en 111.111.111 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 4.888x, com uma taxa de acerto de 1 em 111.111.111 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.50",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.888x con una tasa de apariciÃ³n de 1 en 200.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 4.888x, com uma taxa de acerto de 1 em 200.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  }
					]
				  },
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 333.333.333 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 333.333.333 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 3,125x with a hitrate of 1 in 200,000,000 and can be achieved on the base game',
									es:'El premio mÃ¡x de este juego es de 3.125x con una tasa de apariciÃ³n de 1 en 200.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.125x, com uma taxa de acerto de 1 em 200.000.000 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 3,125x with a hitrate of 1 in 250,000,000 and can be achieved on the base game',
									es:'El premio mÃ¡x de este juego es de 3.125x con una tasa de apariciÃ³n de 1 en 250.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.125x, com uma taxa de acerto de 1 em 250.000.000 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								...
								},
								{
									...
									es:'El premio mÃ¡x de este juego es de 3.125x con una tasa de apariciÃ³n de 1 en 166.666.667 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.125x, com uma taxa de acerto de 1 em 166.666.667 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 10.000X con una posibilidad de conseguirlo de 1 sobre 333.333.333',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 10.000X, com uma possibilidade de ser obtido de 1 em 333.333.333',
								},
								{
									en:'The max prize in this game is 6,667X with a chance to hit of 1 in 142,857,143',
									es:'El premio mÃ¡x de este juego es 6.667X con una posibilidad de conseguirlo de 1 sobre 142.857.143',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 6.667X, com uma possibilidade de ser obtido de 1 em 142.857.143',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 10.000X con una posibilidad de conseguirlo de 1 sobre 500.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 10.000X, com uma possibilidade de ser obtido de 1 em 500.000.000',
								},
								{
									en:'The max prize in this game is 6,667X with a chance to hit of 1 in 125,000,000',
									es:'El premio mÃ¡x de este juego es 6.667X con una posibilidad de conseguirlo de 1 sobre 125.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 6.667X, com uma possibilidade de ser obtido de 1 em 125.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 10.000X con una posibilidad de conseguirlo de 1 sobre 1.000.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 10.000X, com uma possibilidade de ser obtido de 1 em 1.000.000.000',
								},
								{
									en:'The max prize in this game is 6,667X with a chance to hit of 1 in 500,000,000',
									es:'El premio mÃ¡x de este juego es 6.667X con una posibilidad de conseguirlo de 1 sobre 500.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 6.667X, com uma possibilidade de ser obtido de 1 em 500.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 5.000X',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 5.000X',
								},
								{
									en:'The max prize in this game is 4,167X',
									es:'El premio mÃ¡x de este juego es 4.167X',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 4.167X',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								...
								},
								{
									...
									es:'El premio mÃ¡x de este juego es 3.334X',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 3.334X',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 14.000x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 14.000x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.54",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 14.000x con una tasa de apariciÃ³n de 1 en 142.857.143 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 14.000x, com uma taxa de acerto de 1 em 142.857.143 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 15.000x con una tasa de apariciÃ³n de 1 en 166.666.667 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 15.000x, com uma taxa de acerto de 1 em 166.666.667 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 15.000x con una tasa de apariciÃ³n de 1 en 333.333.333 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 15.000x, com uma taxa de acerto de 1 em 333.333.333 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.966x con una tasa de apariciÃ³n de 1 en 2.000.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 5.966x, com uma taxa de acerto de 1 em 2.000.000.000 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 4,772x, with a hitrate 1 in 1,000,000,000 and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 4.772x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 4.772x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 3.000x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 3.000x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
					...
					"configs": [
					  {
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								{
									en:'The max prize in this game is 5,000x',
									es:'El premio mÃ¡x de este juego es de 5.000x',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 5.000x',
								},
								{
									en:'The max prize in this game is 4,000x',
									es:'El premio mÃ¡x de este juego es de 4.000x',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 4.000x',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.000x y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 5.000x e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 4,000x and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 4.000x y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 4.000x e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 125.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 125.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 6.100x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 6.100x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 200.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 200.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 90.909.091 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 90.909.091 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 166.666.667 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 166.666.667 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
					...
					"configs": [
					  ...
					  },
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 500.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						},
						"hideWinLimitPaytable" : true
					  },
					]
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 5.000X con una posibilidad de conseguirlo de 1 sobre 1.000.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 5.000X, com uma possibilidade de ser obtido de 1 em 1.000.000.000',
								},
								{
									en:'The max prize in this game is 3,300X with a chance to hit of 1 in 333,333,333',
									es:'El premio mÃ¡x de este juego es 3.300X con una posibilidad de conseguirlo de 1 sobre 333.333.333',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 3.300X, com uma possibilidade de ser obtido de 1 em 333.333.333',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es 5.000X con una posibilidad de conseguirlo de 1 sobre 500.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 5.000X, com uma possibilidade de ser obtido de 1 em 500.000.000',
								},
								{
									en:'The max prize in this game is 3,300X with a chance to hit of 1 in 200,000,000',
									es:'El premio mÃ¡x de este juego es 3.300X con una posibilidad de conseguirlo de 1 sobre 200.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 3.300X, com uma possibilidade de ser obtido de 1 em 200.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								...
								},
								{
									...
									es:'El premio mÃ¡x de este juego es 3.300X con una posibilidad de conseguirlo de 1 sobre 125.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 3.300X, com uma possibilidade de ser obtido de 1 em 125.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
						{
							...
							"customText" : {
								...
								es:`El premio mÃ¡ximo de este juego es de 1.200x con una tasa de apariciÃ³n de 1 en 257.666 y puede alcanzarse al elegir la opciÃ³n de tiradas gratis con "THE BIG CATCH"`,
								pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 1.200X com uma taxa de acerto de 1 em 257.666 e pode ser ganho ao escolher o modo "THE BIG CATCH" de jogadas grÃ¡tis`
							},
							"hideWinLimitPaytable" : true
						},
						{
							...
							"customText" : {
								...
								es:`El premio mÃ¡ximo de este juego es de 1.200x con una tasa de apariciÃ³n de 1 en 261.780 y puede alcanzarse al elegir la opciÃ³n de tiradas gratis con "THE BIG CATCH"`,
								pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 1.200X com uma taxa de acerto de 1 em 261.780 e pode ser ganho ao escolher o modo "THE BIG CATCH" de jogadas grÃ¡tis`
							},
							"hideWinLimitPaytable" : true
						},
						{
							...
							"customText" : {
								...
								es:`El premio mÃ¡ximo de este juego es de 1.200x con una tasa de apariciÃ³n de 1 en 255.951 y puede alcanzarse al elegir la opciÃ³n de tiradas gratis con "THE BIG CATCH"`,
								pt:`O prÃªmio mÃ¡ximo deste jogo Ã© de 1.200X com uma taxa de acerto de 1 em 255.951 e pode ser ganho ao escolher o modo "THE BIG CATCH" de jogadas grÃ¡tis`
							},
							"hideWinLimitPaytable" : true
						},
					]
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 1.050x con una tasa de apariciÃ³n de 1 en 250.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 1.050x, com uma taxa de acerto de 1 em 250.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.34",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 1.208x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 1.208x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.29",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 1.208x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 1.208x, com uma taxa de acerto de 1 em 500.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  }
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.631x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 5.631x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.50",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.756x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 5.756x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.50",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 6.046x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 6.046x, com uma taxa de acerto de 1 em 500.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  }
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 6.000x con una tasa de apariciÃ³n de 1 en 200.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 6.000x, com uma taxa de acerto de 1 em 200.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.52",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 6.000x con una tasa de apariciÃ³n de 1 en 83.333.333 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 6.000x, com uma taxa de acerto de 1 em 83.333.333 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  }
					]
				},
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								...
								},
								{
									...
									es:'El premio mÃ¡x de este juego es de 3.334x y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 3.334x e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 111.111.111 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 111.111.111 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.09",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 142.857.143 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 142.857.143 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "96.06",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 250.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 250.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 5.000x e pode ser alcanÃ§ado no jogo base`
						  }
					  },
					  {
						"rtp": "95.02",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:'El premio mÃ¡x de este juego es de 7.500x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base',
							pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 7.500x, com uma taxa de acerto de 1 em 500.000.000 e pode ser alcanÃ§ado durante o jogo base',
						}
					  },
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 15.000x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 15.000x e pode ser alcanÃ§ado no jogo base`
						}
					  },
					  {
						"rtp": "95.07",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 3.000x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 3.000x e pode ser alcanÃ§ado no jogo base`
						}
					  },
					  {
						"rtp": "95.04",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.000x con una tasa de apariciÃ³n de 1 en 166.666.667 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 4.000x, com uma taxa de acerto de 1 em 166.666.667 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.53",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.000x con una tasa de apariciÃ³n de 1 en 250.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 4.000x, com uma taxa de acerto de 1 em 250.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },					  {
						"rtp": "96.49",
						"max_rnd_win": 4000,
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.000x con una tasa de apariciÃ³n de 1 en 200.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 4.000x, com uma taxa de acerto de 1 em 200.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					]
				},
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.000x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 10.000x e pode ser alcanÃ§ado no jogo base`
						  }
					  },
					  {
						"rtp": "94.99",
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 4.000x y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© 4.000x e pode ser alcanÃ§ado no jogo base`
						  }
					  },
					  {
						"rtp": "95.50",
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 12.000x con una tasa de apariciÃ³n de 1 en 83.333.333 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 12.000x, com uma taxa de acerto de 1 em 83.333.333 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 10,000x and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 10.000x y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 10.000x e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 12.000x con una tasa de apariciÃ³n de 1 en 71.428.571 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 12.000x, com uma taxa de acerto de 1 em 71.428.571 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 10,000x and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 10.000x y se puede conseguir en el juego base',
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 12.000x y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© 12.000x e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 10,000x, with a hitrate 1 in 76,923,077 and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 10.000x con una tasa de apariciÃ³n de 1 en 76.923.077 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 10.000x, com uma taxa de acerto de 1 em 76.923.077 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 2.000.000.000 y se puede conseguir en el juego base`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 2.000.000.000 e pode ser alcanÃ§ado durante o jogo base`,
						}
					  },
					  {
						"rtp": "95.46",
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 2.500x con una tasa de apariciÃ³n de 1 en 500.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 2.500x, com uma taxa de acerto de 1 em 500.000.000 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 2,000x, with a hitrate 1 in 1,000,000,000 and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 2.000x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 2.000x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 2.500x con una tasa de apariciÃ³n de 1 en 1.000.000.000 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 2.500x, com uma taxa de acerto de 1 em 1.000.000.000 e pode ser alcanÃ§ado durante o jogo base',
								},
								{
									en:'The max prize in this game is 2,000x, with a hitrate 1 in 166,666,667 and can be achieved during the base game',
									es:'El premio mÃ¡x de este juego es de 2.000x con una tasa de apariciÃ³n de 1 en 166.666.667 y se puede conseguir en el juego base',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 2.000x, com uma taxa de acerto de 1 em 166.666.667 e pode ser alcanÃ§ado durante o jogo base',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 3.000x',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.000x',
								},
								{
									en:'The max prize in this game is 2,000x',
									es:'El premio mÃ¡x de este juego es de 2.000x',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 2.000x',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 1.250.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 1.250.000.000',
								},
								{
									en:'The max prize in this game is 3,334x, with a hitrate 1 in 1,000,000,000',
									es:'El premio mÃ¡x de este juego es de 3.334x con una tasa de apariciÃ³n de 1 en 1.000.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.334x, com uma taxa de acerto de 1 em 1.000.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
						...
						var OnBetLevelChanged = function()
						{
							var texts = [
								...
								},
								{
									...
									es:'El premio mÃ¡x de este juego es de 3.334x con una tasa de apariciÃ³n de 1 en 250.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.334x, com uma taxa de acerto de 1 em 250.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
							...
							var texts = [
								{
									...
									es:'El premio mÃ¡x de este juego es de 5.000x con una tasa de apariciÃ³n de 1 en 5.000.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 5.000x, com uma taxa de acerto de 1 em 5.000.000.000',
								},
								{
									en:'The max prize in this game is 3,334x, with a hitrate 1 in 500,000,000',
									es:'El premio mÃ¡x de este juego es de 3.334x con una tasa de apariciÃ³n de 1 en 500.000.000',
									pt:'O prÃªmio mÃ¡ximo neste jogo Ã© de 3.334x, com uma taxa de acerto de 1 em 500.000.000',
								}
							];
							var bls = XT.GetObject(Vars.BetLevelSettings);
							if (bls != null)
					...
					"configs": [
					  {
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 5.824x con una tasa de apariciÃ³n de 1 en 1.000.000.000`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 5.824x, com uma taxa de acerto de 1 em 1.000.000.000`,
						}
					  },
					  {
						"rtp": "95.52",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.303x con una tasa de apariciÃ³n de 1 en 1.000.000.000`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.303x, com uma taxa de acerto de 1 em 1.000.000.000`,
						}
					  },
					  {
						"rtp": "96.53",
						...
						"customText" : {
							...
							es:`El premio mÃ¡x de este juego es de 10.602x con una tasa de apariciÃ³n de 1 en 1.000.000.000`,
							pt:`O prÃªmio mÃ¡ximo neste jogo Ã© de 10.602x, com uma taxa de acerto de 1 em 1.000.000.000`,
						}
					  },
					]
				},
	...
	apply: function()
	{
		...
		var OnXTGameInit = function()
		{
			...
			var warningTextMap = {
				...
				ar: `ÙŠÙ…ÙƒÙ†Ùƒ Ø´Ø±Ø§Ø¡ Ø¯ÙˆØ±Ø§Øª Ù…Ø¬Ø§Ù†ÙŠØ© Ù…Ù† ${betValue} ÙˆÙ…Ø§ ÙÙˆÙ‚ Ø£Ùˆ Ø²ÙŠØ§Ø¯Ø© Ø±Ù‡Ø§Ù†Ùƒ Ø¥Ù„Ù‰ ${minBFValue} Ù„Ù„Ø¯ÙˆØ±Ø§Ù†`,
				bg: `"ÐœÐžÐ–Ð•Ð¢Ð• Ð”Ð Ð—ÐÐšÐ£ÐŸÐ˜Ð¢Ð• Ð‘Ð•Ð—ÐŸÐ›ÐÐ¢ÐÐ˜ Ð—ÐÐ’ÐªÐ Ð¢ÐÐÐ˜Ð¯ ÐžÐ¢ ${minBFValue} Ð˜ ÐŸÐžÐ’Ð•Ð§Ð• Ð˜Ð›Ð˜ Ð”Ð Ð£Ð’Ð•Ð›Ð˜Ð§Ð˜Ð¢Ð• Ð—ÐÐ›ÐžÐ“Ð Ð¡Ð˜ Ð”Ðž ${betValue}, Ð—Ð Ð”Ð Ð—ÐÐ’ÐªÐ Ð¢Ð˜Ð¢Ð•"`,
				cs: `MÅ®Å½ETE SI KOUPIT ROZTOÄŒENÃ ZDARMA OD ${minBFValue} A VÃÅ E NEBO ZVÃÅ IT SÃZKU NA ${betValue} A ROZTOÄŒIT SE`,
				da: `DU KAN KÃ˜BE GRATIS SPINS FRA ${minBFValue} OG DEROVER ELLER Ã˜GE DIN INDSATS TIL ${betValue} FOR AT SPINNE`,
				de: `"SIE KÃ–NNEN KOSTENLOSE DREHUNGEN AB ${minBFValue} ERWERBEN ODER IHREN EINSATZ AUF ${betValue} ERHÃ–HEN, UM ZU DREHEN"`,
				el: `ÎœÎ ÎŸÎ¡Î•ÎŠÎ¤Î• ÎÎ‘ Î‘Î“ÎŸÎ¡Î†Î£Î•Î¤Î• Î”Î©Î¡Î•Î†Î Î Î•Î¡Î™Î£Î¤Î¡ÎŸÎ¦ÎˆÎ£ Î‘Î ÎŒ ${minBFValue} ÎšÎ‘Î™ Î Î†ÎÎ© Î‰ ÎÎ‘ Î‘Î¥ÎžÎ‰Î£Î•Î¤Î• Î¤ÎŸ Î ÎŸÎÎ¤Î†Î¡Î™Î£ÎœÎ† Î£Î‘Î£ Î£Î• ${betValue} Î“Î™Î‘ Î Î•Î¡Î™Î£Î¤Î¡ÎŸÎ¦ÎˆÎ£.`,
				es: `PUEDES COMPRAR TIRADAS GRATIS A PARTIR DE ${minBFValue} O AUMENTAR TU APUESTA A ${betValue} PARA GIRAR`,
				et: `SAAD OSTA TASUTA KEERUTUSI ALATES ${minBFValue} JA ROHKEM VÃ•I SUURENDADA OMA PANUST ${betValue} KEERUTUSTE TEGEMISEKS.`,
				fa: `Ø´Ù…Ø§ Ù…ÛŒâ€ŒØªÙˆØ§Ù†ÛŒØ¯ Ø§Ø² ${betValue} Ø¨Ù‡ Ø¨Ø§Ù„Ø§ Ú†Ø±Ø®Ø´â€ŒÙ‡Ø§ÛŒ Ø±Ø§ÛŒÚ¯Ø§Ù† Ø¨Ø®Ø±ÛŒØ¯ ÛŒØ§ Ø´Ø±Ø· Ø®ÙˆØ¯ Ø±Ø§ Ø¨Ù‡ ${minBFValue} Ø§ÙØ²Ø§ÛŒØ´ Ø¯Ù‡ÛŒØ¯ ØªØ§ Ú†Ø±Ø®Ø´ Ú©Ù†ÛŒØ¯.`,
				fi: `VOIT OSTAA ILMAISKIERROKSIA ${minBFValue}:STÃ„ ALKAEN TAI NOSTAA PANOKSESI ${betValue}:EEN PYÃ–RÃ„YTYSTÃ„ VARTEN.`,
				fr: `VOUS POUVEZ ACHETER DES TOURS GRATUITS Ã€ PARTIR DE ${minBFValue} OU AUGMENTER VOTRE MISE Ã€ ${betValue} POUR TOURNER.`,
				hr: `MOÅ½ETE KUPITI BESPLATNE VRTNJE OD ${minBFValue} ILI POVIÅ AVATI ULOG NA ${betValue} ZA VRNJU`,
				hu: `"INGYENES PÃ–RGETÃ‰SEKET VÃSÃROLHATSZ ${minBFValue}-TÅL Ã‰S FELETTE, VAGY NÃ–VELHETED A TÃ‰TEDET ${betValue}-IG A PÃ–RGETÃ‰SHEZ."`,
				hy: `Ô´Õ¸Ö‚Ö„ Õ¯Õ¡Ö€Õ¸Õ² Õ¥Ö„ Õ£Õ¶Õ¥Õ¬ Õ¡Õ¶Õ¾Õ³Õ¡Ö€ ÕºÕ¿Õ¸Ö‚ÕµÕ¿Õ¶Õ¥Ö€ ${minBFValue} Ö‡ Õ¡Õ¾Õ¥Õ¬Õ« Õ¯Õ¡Õ´ Õ¢Õ¡Ö€Õ±Ö€Õ¡ÖÕ¶Õ¥Õ¬ Õ±Õ¥Ö€ Õ­Õ¡Õ²Õ¡Õ¤Ö€Õ¸Ö‚ÕµÖ„Õ¨ Õ´Õ«Õ¶Õ¹Ö‡ ${betValue} ÕºÕ¿Õ¿Õ¾Õ¥Õ¬Õ¸Ö‚ Õ°Õ¡Õ´Õ¡Ö€:`,
				id: `ANDA BISA BELI SPIN GRATIS MULAI DARI ${minBFValue} DAN LEBIH ATAU NAIKKAN TARUHAN ANDA KE ${betValue} UNTUK SPIN`,
				it: `Ãˆ POSSIBILE ACQUISTARE GIRI GRATUITI A PARTIRE DA ${minBFValue} O AUMENTARE LA PUNTATA A ${betValue} PER GIRARE`,
				ja: `${minBFValue}ä»¥ä¸Šã®ãƒ•ãƒªãƒ¼ã‚¹ãƒ”ãƒ³ã‚’è³¼å…¥ã™ã‚‹ã‹ã€ãƒ™ãƒƒãƒˆé¡ã‚’${betValue}ã¾ã§å¢—ã‚„ã—ã¦ã‚¹ãƒ”ãƒ³ã™ã‚‹ã“ã¨ãŒã§ãã‚‹ã€‚`,
				ka: `áƒ¥áƒ•áƒ”áƒœ áƒ¨áƒ”áƒ’áƒ˜áƒ«áƒšáƒ˜áƒáƒ— áƒ¨áƒ”áƒ˜áƒ«áƒ˜áƒœáƒáƒ— áƒ£áƒ¤áƒáƒ¡áƒ áƒ¡áƒžáƒ˜áƒœáƒ”áƒ‘áƒ˜ ${minBFValue}-áƒ“áƒáƒœ áƒáƒœ áƒ’áƒáƒ–áƒáƒ áƒ“áƒáƒ— áƒ—áƒ¥áƒ•áƒ”áƒœáƒ˜ áƒ¤áƒ¡áƒáƒœáƒ˜ ${betValue}-áƒ›áƒ“áƒ” áƒ¡áƒžáƒ˜áƒœáƒ”áƒ‘áƒ˜áƒ¡áƒ—áƒ•áƒ˜áƒ¡.`,
				ko: `${minBFValue} ì´ìƒì—ì„œ ë¬´ë£Œ ìŠ¤í•€ì„ êµ¬ë§¤í•˜ê±°ë‚˜ ${betValue}ìœ¼ë¡œ ë² íŒ…ì„ ëŠ˜ë ¤ ìŠ¤í•€ì„ ëŒë¦´ ìˆ˜ ìžˆìŠµë‹ˆë‹¤.`,
				lt: `"GALITE NUSIPIRKTI NEMOKAMÅ² SUKIMÅ² NUO ${minBFValue} IR DAUGIAU ARBA PADIDINTI STATYMÄ„ IKI ${betValue}, KAD GALÄ–TUMÄ–TE SUKTI"`,
				lv: `"JÅªS VARAT IEGÄ€DÄ€TIES BEZMAKSAS GRIEZIENUS NO ${minBFValue} UN VAIRÄ€K VAI PALIELINÄ€T LIKMI LÄªDZ ${betValue}, LAI GRIEZTOS."`,
				...
				no: `DU KAN KJÃ˜PE GRATISSPINN FRA ${minBFValue} OG OPPOVER ELLER Ã˜KE INNSATSEN DIN TIL ${betValue} FOR Ã… SPINNE`,
				pl: `"MOÅ»ESZ KUPIÄ† DARMOWE OBROTY OD ${minBFValue} LUB ZWIÄ˜KSZYÄ† STAWKÄ˜ DO ${betValue}, ABY ZAKRÄ˜CIÄ†."`,
				pt: `PODE COMPRAR RODADAS GRÃTIS A PARTIR DE ${minBFValue} OU AUMENTAR A SUA APOSTA PARA ${betValue} PARA RODAR`,
				ru: `Ð’Ð« ÐœÐžÐ–Ð•Ð¢Ð• ÐšÐ£ÐŸÐ˜Ð¢Ð¬ Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ« ÐžÐ¢ ${minBFValue} Ð˜ Ð’Ð«Ð¨Ð• Ð˜Ð›Ð˜ Ð£Ð’Ð•Ð›Ð˜Ð§Ð˜Ð¢Ð¬ Ð¡Ð¢ÐÐ’ÐšÐ£ Ð”Ðž ${betValue} ÐÐ Ð¡ÐŸÐ˜Ð`,
				sk: `MÃ”Å½ETE SI KÃšPIÅ¤ ROZTOÄŒENIE ZDARMA OD ${minBFValue} A VYÅ Å IE ALEBO ZVÃÅ IÅ¤ SVOJU STÃVKU NA ${betValue} A ROZTOÄŒIÅ¤`,
				sr: `MOÅ½ETE KUPITI BESPLATNE OKRETE OD ${minBFValue} ILI POVEÄ†ATI SVOJU OPKLADU NA ${betValue} ZA OKRETANJE`,
				sv: `DU KAN KÃ–PA GRATISSPINN FRÃ…N ${minBFValue} OCH HÃ–GRE ELLER Ã–KA DIN INSATS TILL ${betValue} FÃ–R ATT SPINNA`,
				th: `à¸„à¸¸à¸“à¸ªà¸²à¸¡à¸²à¸£à¸–à¸‹à¸·à¹‰à¸­à¸à¸²à¸£à¸«à¸¡à¸¸à¸™à¸Ÿà¸£à¸µà¹„à¸”à¹‰à¸ˆà¸²à¸ ${minBFValue} à¸‚à¸¶à¹‰à¸™à¹„à¸› à¸«à¸£à¸·à¸­à¹€à¸žà¸´à¹ˆà¸¡à¸à¸²à¸£à¹€à¸”à¸´à¸¡à¸žà¸±à¸™à¸‚à¸­à¸‡à¸„à¸¸à¸“à¹€à¸›à¹‡à¸™ ${betValue} à¹€à¸žà¸·à¹ˆà¸­à¸«à¸¡à¸¸à¸™`,
				tr: `${minBFValue} VE ÃœZERI ÃœCRETSIZ DÃ–NDÃœRME SATIN ALABILIR VEYA DÃ–NDÃœRMEK IÃ‡IN BAHSINIZI ${betValue}'E YÃœKSELTEBILIRSINIZ`,
				uk: `Ð’Ð˜ ÐœÐžÐ–Ð•Ð¢Ð• ÐšÐ£ÐŸÐ˜Ð¢Ð˜ Ð‘Ð•Ð—ÐšÐžÐ¨Ð¢ÐžÐ’ÐÐ† ÐžÐ‘Ð•Ð Ð¢ÐÐÐÐ¯ Ð’Ð†Ð” ${minBFValue} Ð† Ð’Ð˜Ð©Ð• ÐÐ‘Ðž Ð—Ð‘Ð†Ð›Ð¬Ð¨Ð˜Ð¢Ð˜ Ð¡Ð¢ÐÐ’ÐšÐ£ Ð”Ðž ${betValue} ÐÐ ÐžÐ‘Ð•Ð Ð¢ÐÐÐÐ¯`,
				vi: `Báº N CÃ“ THá»‚ MUA VÃ’NG QUAY MIá»„N PHÃ Tá»ª ${minBFValue} TRá»ž LÃŠN HOáº¶C TÄ‚NG CÆ¯á»¢C Cá»¦A Báº N LÃŠN ${betValue} Äá»‚ QUAY`,
				zh: `æ‚¨å¯ä»¥è´­ä¹° ${minBFValue} åŠä»¥ä¸Šçš„å…è´¹æ—‹è½¬ï¼Œæˆ–å°†æŠ•æ³¨é¢æé«˜åˆ° ${betValue} ä»¥è¿›è¡Œæ—‹è½¬`,
				zt: `æ‚¨å¯ä»¥å¾ž ${minBFValue} é–‹å§‹è³¼è²·å…è²»æ—‹è½‰ï¼Œæˆ–è€…å°‡æ‚¨çš„æŠ•æ³¨å¢žåŠ åˆ° ${betValue} é€²è¡Œæ—‹è½‰`
	...
	apply:function()
	{
		this.OnXTGameInit = function()
		{
			var translationDict = 
			{
				...
				"ar": "Ù„Ø§ ØªÙˆØ¬Ø¯ Ø±Ø³Ø§Ø¦Ù„ Ù…ØªØ§Ø­Ø©",
				"bg": "ÐÑÐ¼Ð° Ð½Ð°Ð»Ð¸Ñ‡Ð½Ð¸ ÑÑŠÐ¾Ð±Ñ‰ÐµÐ½Ð¸Ñ",
				"cs": "Å½Ã¡dnÃ© zprÃ¡vy nejsou k dispozici",
				"da": "Ingen beskeder tilgÃ¦ngelige",
				"de": "Keine Nachrichten verfÃ¼gbar",
				"el": "Î”ÎµÎ½ Ï…Ï€Î¬ÏÏ‡Î¿Ï…Î½ Î´Î¹Î±Î¸Î­ÏƒÎ¹Î¼Î± Î¼Î·Î½ÏÎ¼Î±Ï„Î±",
				"es": "No hay mensajes disponibles",
				"et": "SÃµnumid puuduvad",
				"fa": "Ù‡ÛŒÚ† Ù¾ÛŒØ§Ù…ÛŒ Ù…ÙˆØ¬ÙˆØ¯ Ù†ÛŒØ³Øª",
				"fi": "Ei viestejÃ¤ saatavilla",
				"fr": "Aucun message disponible",
				"hr": "Nema dostupnih poruka",
				"hu": "Nincsenek elÃ©rhetÅ‘ Ã¼zenetek",
				"hy": "Õ€Õ¡Õ½Õ¡Õ¶Õ¥Õ¬Õ« Õ°Õ¡Õ²Õ¸Ö€Õ¤Õ¡Õ£Ö€Õ¸Ö‚Õ©ÕµÕ¸Ö‚Õ¶Õ¶Õ¥Ö€ Õ¹Õ¯Õ¡Õ¶",
				"id": "Tidak ada pesan yang tersedia",
				"it": "Nessun messaggio disponibile",
				"ja": "ãƒ¡ãƒƒã‚»ãƒ¼ã‚¸ã¯ã‚ã‚Šã¾ã›ã‚“",
				"ka": "áƒáƒ  áƒáƒ áƒ˜áƒ¡ áƒ¨áƒ”áƒ¢áƒ§áƒáƒ‘áƒ˜áƒœáƒ”áƒ‘áƒ”áƒ‘áƒ˜",
				"ko": "ì‚¬ìš© ê°€ëŠ¥í•œ ë©”ì‹œì§€ê°€ ì—†ìŠµë‹ˆë‹¤.",
				"lt": "PraneÅ¡imÅ³ nÄ—ra",
				"lv": "Nav pieejami ziÅ†ojumi",
				"ms": "Tiada mesej tersedia",
				"nl": "Geen berichten beschikbaar",
				"no": "Ingen meldinger tilgjengelig",
				"pl": "Brak dostÄ™pnych wiadomoÅ›ci",
				"pt": "NÃ£o hÃ¡ mensagens disponÃ­veis",
				"ro": "Niciun mesaj",
				"ru": "Ð¡Ð¾Ð¾Ð±Ñ‰ÐµÐ½Ð¸Ñ Ð½ÐµÐ´Ð¾ÑÑ‚ÑƒÐ¿Ð½Ñ‹",
				"sk": "Å½iadne sprÃ¡vy nie sÃº k dispozÃ­cii",
				"sr": "Nema dostupnih poruka",
				"sv": "Inga meddelanden tillgÃ¤ngliga",
				"th": "à¹„à¸¡à¹ˆà¸¡à¸µà¸‚à¹‰à¸­à¸„à¸§à¸²à¸¡",
				"tr": "Mesaj mevcut deÄŸil",
				"uk": "ÐÐµÐ¼Ð°Ñ” Ð´Ð¾ÑÑ‚ÑƒÐ¿Ð½Ð¸Ñ… Ð¿Ð¾Ð²Ñ–Ð´Ð¾Ð¼Ð»ÐµÐ½ÑŒ",
				"vi": "KhÃ´ng cÃ³ tin nháº¯n nÃ o",
				"zh": "æ— ç•™è¨€",
				"zt": "ç„¡è¨Šæ¯å¯ç”¨"
			}
...
UHTPatch({
	...
	apply:function()
	{
		this.OnXTGameInit = function()
		{
			...
			{
				"en": "FREE BONUS SPINS",
				"ar": "Ù„ÙØ§Øª Ø§Ù„Ù…ÙƒØ§ÙØ£Ø© Ø§Ù„Ù…Ø¬Ø§Ù†ÙŠØ©",
				"bg": "Ð‘Ð•Ð—ÐŸÐ›ÐÐ¢ÐÐ˜ Ð‘ÐžÐÐ£Ð¡ Ð—ÐÐ’ÐªÐ Ð¢ÐÐÐ˜Ð¯",
				"cs": "BONUSOVÃ OTOÄŒENÃ ZDARMA",
				"da": "GRATIS BONUSSPINS",
				"de": "GRATIS-BONUS-SPINS",
				"el": "Î”Î©Î¡Î•Î‘Î Î Î•Î¡Î™Î£Î¤Î¡ÎŸÎ¦Î•Î£ ÎœÎ ÎŸÎÎŸÎ¥Î£",
				"es": "TIRADAS BONUS GRATIS",
				"et": "TASUTA BOONUSKEERUTUSED",
				"fa": "Ú†Ø±Ø®Ø´ Ù¾Ø§Ø¯Ø§Ø´ Ø±Ø§ÛŒÚ¯Ø§Ù†",
				"fi": "ILMAISET BONUSPYÃ–RÃ„YTYKSET",
				"fr": "SPINS BONUS GRATUITS",
				"hr": "BESPLATNE BONUS VRTNJE",
				"hu": "INGYENES BÃ“NUSZ PÃ–RGETÃ‰SEK",
				"hy": "Ô±Õ†ÕŽÕƒÔ±Õ Ô²ÕˆÕ†ÕˆÕ’ÕÔ±Õ…Ô»Õ† ÕÕŠÔ»Õ†Õ†ÔµÕ",
				"id": "PUTARAN BONUS GRATIS",
				"it": "GIRI BONUS GRATIS",
				"ja": "ãƒ•ãƒªãƒ¼ãƒœãƒ¼ãƒŠã‚¹ã‚¹ãƒ”ãƒ³",
				"ka": "á²£á²¤á²á²¡á² á²‘á²á²œá²£á²¡ á²“á²á²¢á² á²˜á²á²šá²”á²‘á²”á²‘á²˜",
				"ko": "ë¬´ë£Œ ë³´ë„ˆìŠ¤ ìŠ¤í•€",
				...
				"pt": "RODADAS BÃ“NUS GRÃTIS",
				"ro": "ROTIRI GRATUITE BONUS",
				"ru": "Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð‘ÐžÐÐ£Ð¡ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ«",
				"sk": "VOÄ½NÃ‰ BONUSOVÃ‰ TOÄŒENIA",
				"sr": "BESPLATNI BONUS SPINOVI",
				"sv": "GRATIS BONUSPINN",
				"th": "à¸ªà¸›à¸´à¸™à¹‚à¸šà¸™à¸±à¸ªà¸Ÿà¸£à¸µ",
				"tr": "ÃœCRETSIZ BONUS SPINLER",
				"uk": "Ð‘Ð•Ð—ÐšÐžÐ¨Ð¢ÐžÐ’ÐÐ† Ð‘ÐžÐÐ£Ð¡ÐÐ† ÐžÐ‘Ð•Ð Ð¢ÐÐÐÐ¯",
				"vi": "VÃ’NG QUAY THÆ¯á»žNG MIá»„N PHÃ",
				"zh": "å…è´¹å¥–åŠ±æ—‹è½¬",
				"zt": "å…è²»çŽå‹µæ—‹è½‰"
			}
				...
				if (t != null)
				{
					...
					if (label != null)
					{
						...
						if (UHT_CONFIG.LANGUAGE == "pt" && ServerOptions.jurisdiction == "BR")
							textPayload = "SESSÃ•ES DE RODADAS PROMOCIONAIS";
		...
		BetGridManager.prototype.OnTotalBetsChanged = function(param)
		{
			...
			if (!BetGridTitleChanged)
			{
				...
				if (IsRequired("NOCUR_CBW"))
				{
					...
					var textList = ["CHANGE BET {0}","ØªØºÙŠÙŠØ± Ø§Ù„Ø±Ù‡Ø§Ù† {0}","ÐŸÐ ÐžÐœÐ¯ÐÐ ÐÐ Ð—ÐÐ›ÐžÐ“Ð {0}","ZMÄšNIT SÃZKU {0}","SKIFT INDSATS {0}","{0} EINSATZ Ã„NDERN","Î‘Î›Î›Î‘Î“Î— Î£Î¤ÎŸÎ™Î§Î—ÎœÎ‘Î¤ÎŸÎ£ {0}","CAMBIAR APUESTA {0}","MUUDA PANUST {0}","ØªØºÛŒÛŒØ± Ø´Ø±Ø· {0}","VAIHDA PANOS {0}","CHANGER LA MISE {0}","PROMIJENITE ULOG {0}","{0} TÃ‰T MEGVÃLTOZTATÃSA","Õ“ÕˆÔ½ÔµÔ¼ Ô½Ô±Õ‚Ô±Ô´ÕÕˆÕ’Õ…Õ”Ô¸Õ {0}","UBAH TARUHAN {0}","MODIFICA PUNTATA {0}","ãƒ™ãƒƒãƒˆ{0}ã‚’å¤‰æ›´","áƒ¤áƒ¡áƒáƒœáƒ˜áƒ¡ áƒ¨áƒ”áƒªáƒ•áƒšáƒ {0}","ë² íŒ… {0} ë³€ê²½","KEISTI STATYMÄ„ {0}","MAINÄªT LIKMI {0}","UBAH TARUHAN {0}","INZET WIJZIGEN {0}","ENDRE INNSATS {0}","ZMIEÅƒ ZAKÅAD {0}","ALTERAR APOSTA {0}","Ð˜Ð—ÐœÐ•ÐÐ˜Ð¢Ð¬ Ð¡Ð¢ÐÐ’ÐšÐ£ {0}","ZMENIÅ¤ STÃVKU {0}","PROMENI ULOG {0}","Ã„NDRA INSATS {0}","à¹€à¸›à¸¥à¸µà¹ˆà¸¢à¸™à¹€à¸”à¸´à¸¡à¸žà¸±à¸™ {0}","{0} DEÄžERÄ°NDEKÄ° BAHSÄ° DEÄžÄ°ÅžTÄ°R","Ð—ÐœÐ†ÐÐ˜Ð¢Ð˜ Ð¡Ð¢ÐÐ’ÐšÐ£ {0}","THAY Äá»”I CÆ¯á»¢C {0}","æ›´æ”¹èµŒæ³¨{0}","è®Šæ›´è³­æ³¨{0}"];
						...
						if (arguments[0][i] != null && res[r].id == _number.otoi(arguments[0][i][TournamentProtocol.Dictionary.Details.id]))
						{
							...
							if (dict[TournamentProtocol.Dictionary.Details.entryFeatureSettings] != undefined)
							{
								...
								if (details.entryFeatureSettings["availableGameName"] != undefined)
									details.entryFeatureSettings["availableGameName"] = details.entryFeatureSettings["availableGameName"].replace(/ /g, "Â ");
							}
						}
			}
			return res;
			...
			if (XT.GetString(TournamentVars.PrizeDropWin_FreeRoundsType) == "F")
				if (XT.GetInt(TournamentVars.PrizeDropWin_PrizeType) == TournamentProtocol.PrizeType.FreeRounds && details != null && details.entryFeatureSettings != null && details.entryFeatureSettings["availableGameName"] != null)
				{
					...
					if (availableGameName != undefined)
					{
						...
						var trimmedGameTitle = XT.GetString(Vars.GameTitle).replace("â„¢", "").replace(/\s/g, "").toLowerCase();
	...
	apply:function()
	{
		...
		if (IsRequired("NOJPTW"))
		{
			if (window["UHT_GAME_CONFIG"]["GAME_SYMBOL"].indexOf("vsprg") == 0)
			{
				...
				{
					if (param != "")
						XT.SetString("BiggestWinIDJackpotMinor" , "â€‹")
				};

				this.OnMajorBigWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("BiggestWinIDJackpotMajor" , "â€‹")
				};

				this.OnMegaBigWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("BiggestWinIDJackpotMega" , "â€‹")
				};

				this.OnGrandBigWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("BiggestWinIDJackpotGrand" , "â€‹")
				};

				this.OnMinorLatestWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("LatestWinIDJackpotMinor" , "â€‹")
				};

				this.OnMajorLatestWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("LatestWinIDJackpotMajor" , "â€‹")
				};

				this.OnMegaLatestWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("LatestWinIDJackpotMega" , "â€‹")
				};

				this.OnGrandLatestWinnerId = function(param)
				{
					if (param != "")
						XT.SetString("LatestWinIDJackpotGrand" , "â€‹")
				};
	...
	apply:function()
	{
		var TReplacements =
		{
			...
			pt___BR: [
				...
				{src:"numa posiÃ§Ã£o aleatÃ³ria adicionado ao ecrÃ£.", dst:"adicionado numa posiÃ§Ã£o aleatÃ³ria na tela.", p:true},
				{src:"a caracterÃ­stica RESPIN Ã© accionada e 5 respinas sÃ£o atribuÃ­das.", dst:"o extra RESPIN Ã© acionado e sÃ£o atribuÃ­das 5 novas rodadas.", p:true},
				{src:"Durante a CARACTERÃSTICA DE RESPINA todos os sÃ­mbolos SCATTER permanecem no ecrÃ£ atÃ© ao final da rodada e o nÃºmero de sÃ­mbolos em cada bobina permanece o mesmo que a rotaÃ§Ã£o que desencadeia a caracterÃ­stica.", dst:"Durante o EXTRA DE RESPIN todos os sÃ­mbolos SCATTER permanecem na tela atÃ© ao final da rodada e o nÃºmero de sÃ­mbolos em cada rolo permanece o mesmo que a rodada que desencadeou o extra."},
				{src:"O jogo paga as combinaÃ§Ãµes vencedoras e as rodadas vencedoras normalmente entre cada respina. A rodada termina quando todas as 5 respinas ou um sÃ­mbolo SCATTER estÃ£o presentes em cada carretel.", dst:"O jogo paga as combinaÃ§Ãµes e as rodadas vencedoras, geralmente, entre cada rodada. A rodada termina quando todas as 5 rodadas ou um sÃ­mbolo SCATTER estejam presentes em cada carretel."},

				{src:"Os sÃ­mbolos dÃ£o prÃ©mio",  dst:"Os sÃ­mbolos sÃ£o prÃªmio", p:true},
				...
				{src:"intantÃ¢nea", dst:"instantÃ¢nea", p:true, s:true},
				{src:"as bobinas", dst:"os rolos", p:true, s:true},
				{src:"na bobina", dst:"no rolo", p:true, s:true},

				{src:"prÃ©mio", dst:"prÃªmio", p:true, s:true},
				{src:"actual", dst:"atual", p:true, s:true},
				{src:"accionada", dst:"acionada", p:true, s:true},
				{src:"bÃ³nus", dst:"bÃ´nus", p:true, s:true},
				{src:"ecrÃ£s", dst:"tela", p:true, s:true},
				{src:"prÃ©mios", dst:"prÃªmios", p:true, s:true},
				...
				{src:"rodadas grÃ¡tis da arriscar", dst:"rodadas grÃ¡tis da aposta", p:true, s:true},
				{src:"Antes do inÃ­cio da rodada jogador", dst:"Antes do inÃ­cio da rodada, o jogador", p:true, s:true},
		...
		if (window["UHT_CONFIG"].LANGUAGE == "hy")
		{
			...
			{
				if (copyFrom.text.toLowerCase() == 'Õ®Õ¡Õ°Õ¸Ö‚Õ´')
					copyFrom.text = 'Õ‡Ô±Õ€ÕˆÕ’Õ„';
		...
		if (window["UHT_CONFIG"].LANGUAGE == "de")
		{
			...
			ModificationsManager.CopyFromLabelToLabel = function(copyFrom, copyTo, alsoCopyText, copyEffects)
			{
				if (copyFrom.text == "Alle Symbole zahlenangefangen bei der Ã¤uÃŸerst linken Rolle von links nach rechts fÃ¼r alle nebeneinanderliegenden Rollen.")
		...
		if (window["UHT_CONFIG"].LANGUAGE == "pt")
		{
			...
			ModificationsManager.CopyFromLabelToLabel = function(copyFrom, copyTo, alsoCopyText, copyEffects)
			{
				if (copyFrom.text == "PRIMA A TECLA DE ESPAÃ‡O PARA ROTAÃ‡ÃƒO TURBO")
					copyFrom.text = "PRESSIONE A TECLA DE ESPAÃ‡O PARA ROTAÃ‡ÃƒO TURBO";
				...
				{
					if (copyFrom.text.indexOf("RONDAS DE RODADAS GRÃTIS") != -1)
						copyFrom.text = copyFrom.text.replace("RONDAS DE RODADAS GRÃTIS","SESSÃ•ES DE RODADAS PROMOCIONAIS");
					if (copyFrom.text.indexOf("RODADAS BÃ“NUS GRÃTIS") != -1)
						copyFrom.text = copyFrom.text.replace("RODADAS BÃ“NUS GRÃTIS","SESSÃ•ES DE RODADAS PROMOCIONAIS");
					if (copyFrom.text.indexOf("RONDAS GRÃTIS RESTANTES") != -1)
						copyFrom.text = copyFrom.text.replace("RONDAS GRÃTIS RESTANTES", "RODADAS PROMOCIONAIS RESTANTES");
					if (copyFrom.text.indexOf("CONCLUIU AS SUAS RODADAS BÃ“NUS GRÃTIS") != -1)
						copyFrom.text = copyFrom.text.replace("CONCLUIU AS SUAS RODADAS BÃ“NUS GRÃTIS", "CONCLUIU AS SUAS RODADAS BONUS PROMOCIONAIS");
				}
				MMCFLTL.apply(this, arguments);
			}
		}
		...
		if (window["UHT_CONFIG"].LANGUAGE == "uk")
		{
			...
			ModificationsManager.CopyFromLabelToLabel = function(copyFrom, copyTo, alsoCopyText, copyEffects)
			{
				if (copyFrom.text == "BAC")
					copyFrom.text = "Ð’Ð˜";
				if (copyFrom.text.indexOf("sÐ ÐÐ£ÐÐ”Ð˜") != -1)
					copyFrom.text = copyFrom.text.replace("sÐ ÐÐ£ÐÐ”Ð˜", "Ð ÐÐ£ÐÐ”Ð˜");
				if (copyFrom.text == "Ð¦Ð•Ð™ Ð‘ÐžÐÐ£Ð¡ Ð”ÐžÐ¡Ð¢Ð£ÐŸÐÐ˜Ð™ Ð—Ð")
					copyFrom.text = "Ð¦Ð•Ð™ Ð‘ÐžÐÐ£Ð¡ Ð”ÐžÐ¡Ð¢Ð£ÐŸÐÐ˜Ð™ ÐŸÐ ÐžÐ¢Ð¯Ð“ÐžÐœ";
				if (copyFrom.text == "ÐšÐ Ð•Ð”Ð˜Ð¢")
					copyFrom.text = "Ð‘ÐÐ›ÐÐÐ¡";
				MMCFLTL.apply(this, arguments);
			};
		}
		...
		if (window["UHT_CONFIG"].LANGUAGE == "lt")
		{
			...
			ModificationsManager.CopyFromLabelToLabel = function(copyFrom, copyTo, alsoCopyText, copyEffects)
			{
				if (copyFrom.text == "Nepakanka lÄ—Å¡Å³ statymui. Ä®dÄ—kite pinigÅ³ Ä¯ sÄ…skaita arba sumaÅ¾inkite staymÄ….")
					copyFrom.text = "Nepakanka lÄ—Å¡Å³ statymui. Ä®dÄ—kite pinigÅ³ Ä¯ sÄ…skaita arba sumaÅ¾inkite statymÄ….";
				MMCFLTL.apply(this, arguments);
			};
		}
		...
		if (window["UHT_CONFIG"].LANGUAGE == "zh")
		{
			...
			ModificationsManager.CopyFromLabelToLabel = function(copyFrom, copyTo, alsoCopyText, copyEffects)
			{
				if (copyFrom.text == " å¾Œé–‹å§‹")
					copyFrom.text = " å³å°†å¼€å§‹";
				MMCFLTL.apply(this, arguments);
			};
		}
	},
...
UHTPatch({
	...
	apply:function()
	{
		if (IsRequired("INFAP"))
		{
			...
			{
				if (value != "âˆž" && isInfinityAutoplay)
					XT.SetInt(Vars.AutoplaySpinsLeftDisplayed, "âˆž")
			};
			...
			SystemMessageShowVerifySteps = function()
			{
				...
				if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "ko")
					CustomMsgManager.HandleResponse(null, {custom_msg:JSON.stringify([{
							title:"",
							text:'ì •í’ˆ Pragmatic Play ê²Œìž„ì„ í”Œë ˆì´í•˜ê³  ìžˆëŠ”ì§€ í™•ì¸í•˜ê¸° ìœ„í•´, ë‹¤ìŒê³¼ ê°™ì´ ì§„í–‰í•´ ì£¼ì‹­ì‹œì˜¤.\n \n1. êµ¬ê¸€ì—ì„œ "Pragmatic Play"ê²€ìƒ‰ í›„ ê³µì‹ í™ˆíŽ˜ì´ì§€ì— ì ‘ì†í•˜ì‹­ì‹œì˜¤.\n \n2. Pragmatic Play ê³µì‹ íŽ˜ì´ì§€ì—ì„œ ë…¹ìƒ‰ìœ¼ë¡œ í‘œì‹œ ëœ "ê²Œìž„ ì •í’ˆ ì¸ì¦"ì„ í´ë¦­í•˜ì…”ì„œ ì ˆì°¨ë¥¼ ë”°ë¼ì£¼ì„¸ìš”.',
							nonIntrusive:false,
							options:
							[{	action:"continue",
								label:"OK" }]
		...
		VerifyGameAuthenticityManager.prototype.OpenIframe = function()
		{
			if (!initialized)
			{
				...
				if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "ko")
				{
					if (Globals.isMobile)
					{
						document.getElementsByTagName("h1")[0].innerHTML = 'ê²Œìž„ ì •í’ˆ ì¸ì¦';
						document.getElementsByTagName("h1")[1].innerHTML = 'ê²Œìž„ ì •í’ˆ ì¸ì¦';

						document.getElementById("para1Port").innerHTML =
							document.getElementById("para1").innerHTML =
							'ì´ë¯¸ ì•±ì´ ìžˆëŠ” ê²½ìš° ì•±ì„ ì‹¤í–‰í•˜ì—¬ ìƒˆë¡œìš´ ê²Œìž„ ì¸ì¦ì„ ì‹œìž‘í•˜ì„¸ìš”.'+
							'<br><br>'+
							'ì‹ ê·œ ì‚¬ìš©ìžì˜ ê²½ìš°:'+
							'<br>'+
							'1. ëª¨ë°”ì¼ ê¸°ê¸°ì—ì„œ ì• í”Œ ì•±ìŠ¤í† ì–´ ë˜ëŠ” êµ¬ê¸€ í”Œë ˆì´ ìŠ¤í† ì–´ë¡œ ì´ë™í•©ë‹ˆë‹¤.'+
							'<br>'+
							'2. <span class="orange">PRAGMATIC PLAY ì¸ì¦</span>ì„ ê²€ìƒ‰í•©ë‹ˆë‹¤. ê²Œì‹œìžê°€ <span class="orange">PRAGMATIC PLAY</span>ì¸ì§€ í™•ì¸í•©ë‹ˆë‹¤. ê·¸ë¦¬ê³  <img id="appImgIcon">ì„ í™•ì¸í•´ì£¼ì„¸ìš”'+
							'<br>'+
							'3. ì•±ì„ ì„¤ì¹˜í•˜ê³  ì‹¤í–‰í•©ë‹ˆë‹¤. ë‹¹ì‚¬ íŒ€ì€ 24ì‹œê°„ ì—°ì¤‘ë¬´íœ´ë¡œ ì•ˆë‚´ ë° ì§€ì›ì„ ì œê³µí•©ë‹ˆë‹¤.'+
							'';
					}
					else
					{
				...
				if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "pt")
				{
					if (Globals.isMobile)
					{
						...
						document.getElementById("para1Port").innerHTML =
							document.getElementById("para1").innerHTML =
							'Se vocÃª jÃ¡ tem nosso aplicativo, abra-o para iniciar uma nova verificaÃ§Ã£o de jogo.'+
							'<br><br>'+
							'Para novos usuÃ¡rios:'+
							'<br>'+
							'1. Acesse a Apple App Store ou Google Play Store no seu dispositivo mÃ³vel.'+
							'<br>'+
							'2. Pesquise por <span class="orange">PRAGMATIC PLAY AUTHENTICATOR</span>. Certifique-se de que o publicador seja a <span class="orange">PRAGMATIC PLAY</span> e que o Ã­cone seja parecido com este: '+
							'<img id="appImgIcon">'+
							'<br>'+
							'3. Instale e abra o aplicativo. Nossa equipe estÃ¡ disponÃ­vel 24/7 para orientÃ¡-lo e ajudÃ¡-lo.'+
							'';
					}
					else
					{
...
UHTPatch({
	...
	apply:function()
	{
			...
			this.OnXTGameInit = function()
			{
				if (!Globals.isMobile)
				{
					...
					var newTranslationDesktop = [
						"ÃœCRETSÄ°Z DÃ–NÃœÅž KAZANDINIZ!",
						"OYNAMAYA BAÅžLATMAK Ä°Ã‡Ä°N DEVAM'A BASIN",
						"ÅžANÅžLISINIZ!",
						"ÃœCRETSÄ°Z DÃ–NÃœÅž!",
						"BAHÄ°S",
						"IÃ‡INDE BAÅžLAR"
		...
		if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "th")
		{
			...
			this.OnXTGameInit = function()
			{
				var newTranslation = "à¸‚à¸­à¸‡à¸£à¸²à¸‡à¸§à¸±à¸¥à¸ˆà¸³à¸™à¸§à¸™";
				var localizationRoot = globalRuntime.sceneRoots[1].GetComponentInChildren(LocalizationRoot);
				if (localizationRoot == null)
					return;
			...
			this.OnXTGameInit = function()
			{
				...
				if (!Globals.isMobile)
				{
					...
					if (paytable != null)
					{
						...
						for (var i = 0; i < labels.length; i++)
						{
							if (labels[i].text.trim() == "Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð¥ Ð¡ÐŸÐ˜ÐÐžÐ’")
								labels[i].text = "Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ«";
						}
					}
					...
					if (t != null)
					{
						...
						for (var i = 0; i < labels.length; i++)
						{
							if (labels[i].text.indexOf("Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð‘ÐžÐÐ£Ð¡ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ†") != -1)
								labels[i].text = labels[i].text.replace("Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð‘ÐžÐÐ£Ð¡ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ†","Ð‘Ð•Ð¡ÐŸÐ›ÐÐ¢ÐÐ«Ð• Ð‘ÐžÐÐ£Ð¡ÐÐ«Ð• Ð¡ÐŸÐ˜ÐÐ«")
						}
					}
				}
				else
		...
		if (window["UHT_GAME_CONFIG"]["LANGUAGE"] == "el")
		{
			this.OnXTGameInit = function()
			{
				...
				for (var i = 0; i < labels.length; i++)
				{
					if (labels[i].text.indexOf("ÎšÎ•Î¡Î™Î£Î¤Î•") != -1)
						labels[i].text = labels[i].text.replaceAll("ÎšÎ•Î¡Î™Î£Î¤Î•","ÎšÎ•Î¡Î”Î™Î£Î¤Î•");
				}
			}
			XT.RegisterCallbackEvent(Vars.Evt_Internal_GameInit, this.OnXTGameInit, this);
		}
				...
				if (!Globals.isMobile)
				{
					...
					if (t != null)
					{
						...
						for (var i = 0; i < labels.length; i++)
						{
							...
							if (labels[i].text.indexOf("Â¡HA RECIBIDO") != -1)
								labels[i].text = labels[i].text.replace("Â¡HA RECIBIDO","Â¡HAS RECIBIDO!")
							if (labels[i].text.indexOf("HA RECIBIDO") != -1)
								labels[i].text = labels[i].text.replace("HA RECIBIDO","Â¡HAS RECIBIDO!")
						}
					}
				}
				else
...
function PatchCurrency()
{
	...
	var map=[{c:"BYN",s:"Br"},{c:"PEN",s:"S/."},{c:"VND2",s:"Kâ‚«"},{c:"GEL",s:"â‚¾"}];
