...
namespace vtortola.WebSockets
{
    internal class WebSocketHandshaker
    {
        ...
        //Added by Foresight(2019.09.29)
        public  IPEndPoint ClientEndPointFromProxyV1 { get; set; } 
        ...
        public async Task<WebSocketHandshake> HandshakeAsync(Stream clientStream)
        {
            ...
            try
            {
                ...
                if (_configuration.Options.IsParseProxyV1)
                    handshake.RemoteEndPointFromProxyV1 = this.ClientEndPointFromProxyV1;
            }
            catch(Exception ex)
            {
        ...
        private async Task ReadHttpRequestAsync(Stream clientStream, WebSocketHandshake handshake)
        {
            using (var sr = new StreamReader(clientStream, Encoding.ASCII, false, _configuration.Options.SendBufferSize, true))
            {
                ...
                if (_configuration.Options.IsParseProxyV1)
                {
                    string[] strParts = line.Split(new string[0], StringSplitOptions.RemoveEmptyEntries);
                    if (strParts == null || strParts.Length != 6)
                        throw new Exception("ProxyV1 Parsing Error");

                    try
                    {
                        ClientEndPointFromProxyV1 = new IPEndPoint(IPAddress.Parse(strParts[2]), int.Parse(strParts[4]));
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("ProxyV1 Parsing Error");
