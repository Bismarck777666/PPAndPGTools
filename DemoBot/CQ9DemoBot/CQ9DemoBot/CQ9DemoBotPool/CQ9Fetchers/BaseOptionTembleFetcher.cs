...
{
    public class BaseOptionTembleFetcher : SpinDataFetcher
    {
        private readonly ILoggingAdapter _logger = Logging.GetLogger(Context);
        public BaseOptionTembleFetcher(int proxyIndex, Config config) : base(proxyIndex, config)
        {
        }
        public static new Props Props(int proxyIndex, Config config)
        {
            return Akka.Actor.Props.Create(() => new BaseOptionTembleFetcher(proxyIndex, config));
        }
        ...
        protected override async Task sendFreeSpinOptSelectRequest(int state, int index)
        {
        ...
        {
            _freeSpinStack.Add(JsonConvert.SerializeObject(response));
            bool isGameComplete = Convert.ToBoolean(response.Property("GameComplete").Value);
