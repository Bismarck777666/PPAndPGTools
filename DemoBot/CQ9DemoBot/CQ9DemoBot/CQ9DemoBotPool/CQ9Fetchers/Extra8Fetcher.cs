...
{
    public class Extra8Fetcher : Extra10Fetcher
    {
        public Extra8Fetcher(int proxyIndex, Config config) : base(proxyIndex, config)
        {
        }
        public static new Props Props(int proxyIndex, Config config)
        {
            return Akka.Actor.Props.Create(() => new Extra8Fetcher(proxyIndex, config));
        }
