...
{
    public class OptionMiniFetcher : OptionGameFetcher
    {
        private readonly ILoggingAdapter _logger = Logging.GetLogger(Context);
        public OptionMiniFetcher(int proxyIndex, Config config) : base(proxyIndex, config)
        {
        }
        public static new Props Props(int proxyIndex, Config config)
        {
            return Akka.Actor.Props.Create(() => new OptionMiniFetcher(proxyIndex, config));
        }
