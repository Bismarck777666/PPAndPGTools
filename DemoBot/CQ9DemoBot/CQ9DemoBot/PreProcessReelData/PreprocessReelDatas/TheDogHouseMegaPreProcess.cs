...
{
    public class TheDogHouseMegaPreProcess : SpinDataPreProcess
    {
        ...
        public TheDogHouseMegaPreProcess()
        {
        }
        ...
        private List<FreeOptionSpinData> doPreProcessStartSpin(List<FreeOptionSpinData> fsOptStartList,List<FreeOptionSpinData>[] fsOptFreeListArray)
        {
            ...
            for(int i = 0; i < fsOptStartList.Count; i++)
            {
                ...
                if (startSpinData.Data.Contains("fs_opt=15,1,0~7,1,0"))
                    fsOptStartList[i].FreeSpinType = 0;
                else if (startSpinData.Data.Contains("fs_opt=18,1,0~12,1,0"))
                    fsOptStartList[i].FreeSpinType = 1;
                else if (startSpinData.Data.Contains("fs_opt=25,1,0~15,1,0"))
                    fsOptStartList[i].FreeSpinType = 2;
                else if (startSpinData.Data.Contains("fs_opt=30,1,0~20,1,0"))
                    fsOptStartList[i].FreeSpinType = 3;
            }
            ...
            for (int i = 0; i < fsOptStartList.Count; i++)
            {
                ...
                for(int k = 0; k < OddList.Count; k++)
                {
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        int spinTypeIndex = fsOptStartList[i].FreeSpinType * 2 + j;//200+가 스핀타입
