...
{
    public class JumpHigh2PreProcess : SpinDataPreProcess
    {
        private List<MinMaxItem> OddList = new List<MinMaxItem>
        {
            ...
            new MinMaxItem(1000,1500),  //500~1000
            new MinMaxItem(1500,3000),  //1000~3000
        };
        public JumpHigh2PreProcess()
        {
            _optionCnt = 5;
        }
        public override async Task startPreProcess(SqliteDatabaseWork databaseWork,string strGameName)
        {
            ...
            for (int i = 0; i < _FreeSpinDataList.Count; i++)
            {
                if (_FreeSpinDataList[i].SpinType == 100)
                    ...
                    freeSpinDatas[12].Add(_FreeSpinDataList[i]);
                else if (_FreeSpinDataList[i].SpinType == 213)
                    freeSpinDatas[13].Add(_FreeSpinDataList[i]);
                else if (_FreeSpinDataList[i].SpinType == 214)
                    freeSpinDatas[14].Add(_FreeSpinDataList[i]);
            }
        ...
        private List<FreeOptionSpinData> doPreProcessStartSpin(List<FreeOptionSpinData> fsOptStartList,List<FreeOptionSpinData>[] fsOptFreeListArray)
        {
            ...
            for (int i = 0; i < fsOptStartList.Count; i++)
            {
                int freeSpinGroup = fsOptStartList[i].FreeSpinType;
                ...
                {
                    bool flag = true;
                    for (int j = 0; j < _optionCnt; j++)
                    {
                        int spinTypeIndex = freeSpinGroup * 5 + j;//200+가 스핀타입
