...
{
    public class LuckyFortuneCatProcess : SpinDBPreprocess
    {
        public LuckyFortuneCatProcess(string strFolderName, string strGameName) : base(strFolderName, strGameName)
        {
        ...
        public override async Task preprocessDB()
        {
            using (SQLiteConnection connection = new SQLiteConnection(_strConnString))
            {
                ...
                strCommand = string.Format("UPDATE spins SET spintype=0 WHERE id IN ({0})", string.Join(",", notSuitableIds));
