...
{
    public class JumpFetcher : SpinDataFetcher_1
    {
        protected string _pssid = "";
        public JumpFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin)
        ...
        protected override async Task<List<SpinData>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if (Convert.ToBoolean(response["portmessage"]["featuretriggered"]) && (Convert.ToInt32(response["portmessage"]["Jump_reSpinIndex"]) == 5))
                    {
                        nowFreeSpin = true;
                    }
