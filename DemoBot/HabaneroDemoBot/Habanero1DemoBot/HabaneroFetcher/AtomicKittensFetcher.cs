...
{
    public class AtomicKittensFetcher : SpinDataFetcher_1
    {
        public int _purType = 0;
        public AtomicKittensFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin, int purType)
            : base(strProxyInfo, strProxyUserID, strProxyPassword, strBrandGameId, strClientVersion, realBet, lineCount, betLevel, stake, coin)
        {
            _purType = purType;
        }

        private async Task<string> sendPickRequest(HttpClient httpClient,string gameid,string gssid, int pickIndex)
        {
            ...
            PayLoadPortMessage["pickIndex"] = pickIndex;
        ...
        protected override async Task<List<SpinData>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if (nextgamestate == "kittybonus")
                    {
                        strResponse = await sendPickRequest(httpClient, gameid, gssid, pickIndex);
                        pickIndex++;

                        response    = JsonConvert.DeserializeObject<dynamic>(strResponse);
                        if (!object.ReferenceEquals(response["portmessage"]["AtomicKittens_pickReset"], null))
                            pickIndex = 0;
                    }
                    else
                    {
                    ...
                    nextgamestate = (string)response["portmessage"]["nextgamestate"];
                    if (nextgamestate == "freegame")
                        nowFreeSpin = true;
