...
{
    public class EgytianDreamsDeluxeFetcher : SpinDataFetcher
    {
        protected int       _sequenceType   = 0;
        protected string    _pssid          = "";
        public EgytianDreamsDeluxeFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin)
        ...
        protected override async Task<List<SpinData>> doSpin(HttpClient httpClient)
        {
            ...
            int bufSequenceType = _sequenceType;
            ...
            try
            {
                do
                {
                    ...
                    if (Convert.ToBoolean(response["portmessage"]["featuretriggered"]))
                    {
                        nowFreeSpin = true;
                    }

                    JObject portMessage = response["portmessage"] as JObject;
                    if (!object.ReferenceEquals(portMessage["Jump_bs"], null))
                    {
                        portMessage.Property("Jump_bs").Remove();
                    }

                    string strSpinData = JsonConvert.SerializeObject(portMessage);
                    
                    bufSequenceType = getSequenceFromResponse(portMessage, bufSequenceType);
                    ...
                    if (Convert.ToBoolean(response["portmessage"]["isgamedone"]))
                    {
                        ...
                        spinResponse.SequenceType   = _sequenceType.ToString();
        ...
        private int getSequenceFromResponse(dynamic response, int beforeSequence)
        {
            int increaseVal = 0;
            for (int j = 0; j < response["reels"].Count; j++)
            {
                ...
                for (int k = 0; k < response["reels"][j].Count; k++)
                {
                    ...
                    {
                        increaseVal += (int)Math.Pow(3 , j) + (2 - Convert.ToInt32(response["reels"][j][k]["meta"]["blockIndex"]));
                    }
                }
            }
            return beforeSequence + increaseVal;
        }
    }
