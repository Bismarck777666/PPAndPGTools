...
{
    public class TukTukThailandFetcher : SpinDataFetcher_1
    {
        public int _purType = 0;
        public TukTukThailandFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin, int purType)
        ...
        protected override async Task<string> sendSpinRequest(HttpClient httpClient, string gameid,string gssid)
        {
            ...
            if (string.IsNullOrEmpty(gameid))
            {
                ...
                PayLoadPortMessage["featureBuy"]    = _purType + 2;
            }
            else
            {
        ...
        {
            if(_purType == 0)
                return await base.doSpin(httpClient);
