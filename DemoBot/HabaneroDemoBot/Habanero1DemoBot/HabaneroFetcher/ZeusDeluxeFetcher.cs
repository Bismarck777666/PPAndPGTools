...
{
    public class ZeusDeluxeFetcher : SpinDataFetcher_1
    {
        protected string    _pssid          = "";
        protected int       _purType        = 0;
        protected bool      _hasWildLock    = false;
        public ZeusDeluxeFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin, int purType)
        ...
        protected override async Task<List<SpinData>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if (!object.ReferenceEquals(response["portmessage"]["wildLockList"], null) && response["portmessage"]["wildLockList"].Count > 0)
                        _hasWildLock = true;
                    else
                        _hasWildLock = false;
