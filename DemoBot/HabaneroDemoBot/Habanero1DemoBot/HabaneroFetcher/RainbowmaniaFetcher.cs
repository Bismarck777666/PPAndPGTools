...
{
    public class RainbowmaniaFetcher : SpinDataFetcher_1
    {
        public int _purType = 0;
        public RainbowmaniaFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strBrandGameId, string strClientVersion, double realBet, int lineCount, int betLevel, double stake, int coin, int purType)
                ...
                do
                {
                    ...
                    if (Convert.ToBoolean(response["portmessage"]["isgamedone"]))
                    {
                        ...
                        if(_purType != 0)
                            ...
                            spinResponse.SpinType = nowFreeSpin ? 2 : 0;
