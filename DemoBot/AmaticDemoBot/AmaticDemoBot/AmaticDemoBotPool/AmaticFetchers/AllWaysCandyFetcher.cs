...
{
    public class AllWaysCandyFetcher : AnteFetcher
    {
        protected int _purType = -1;
        public AllWaysCandyFetcher(int proxyIndex, Config config) : base(proxyIndex, config)
        {
        ...
        {
            return Akka.Actor.Props.Create(() => new AllWaysCandyFetcher(proxyIndex, config));
        }

        protected override async Task receiveResponse(AmaPacket packet, string msgStr)
        {
            if(packet.messagetype == (int)MessageType.PurFree)
            {
        ...
        {
            if (packet.messagetype == (long)MessageType.PurFree)
            {
                _nowFreeSpin    = true;
        ...
        //        else
        //        {
        //            PurSpinResponse spinResponse = new PurSpinResponse();
        //            spinResponse.SpinType = 1;
        //            spinResponse.LineType = _lineType;
        //            spinResponse.TotalWin = packet.win;
        //            spinResponse.Response = string.Join("\n", _freeSpinStack);
        //            spinResponse.PurIndex = _purType;
