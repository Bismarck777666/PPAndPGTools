...
{
    class EyeOfCleopatraFetcher : GameSpinDataFetcher
    {
        protected int[] _freeSpinCounts = new int[] { 0, 0, 0, 0, 100, 100, 100, 0, 0, 0, 0, 0 };
        public EyeOfCleopatraFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        {
            if (dicParams["wins"].StartsWith("10,8,6"))
                return 0;
            else if (dicParams["wins"].StartsWith("20,16,12"))
                return 1;
            else if (dicParams["wins"].StartsWith("30,24,18"))
                return 2;
        ...
        protected override int selectFreeOption(int freeSpinType)
        {
            ...
            for(int i = 0; i < 4; i++)
            {
                int index = 4 * freeSpinType + i;
                if (_freeSpinCounts[index] < minCount || minCount == -1)
                {
                    minCount  = _freeSpinCounts[index];
                    minOption = i;
                }
            }
            return minOption;
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        _freeSpinCounts[freeSpinType * FreeSpinOptionCount + freeSpinOption]++;
