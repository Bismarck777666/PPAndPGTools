...
{
    class QueenOfAtlantisFetcher : GameSpinDataFetcher
    {
        public QueenOfAtlantisFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "s")
                    {
                        ...
                        if (strNextAction == "s" && (dicParamValues.ContainsKey("rs_t")  || !dicParamValues.ContainsKey("rs_p"))&& !dicParamValues.ContainsKey("fs")
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        strResponseHistory.Add(combineResponse(dicParamValues, beforeFreeTotalWin));
                        if(strNextAction == "s" && !dicParamValues.ContainsKey("fs"))
                        {
                            SpinResponse response   = new SpinResponse();
                            response.SpinType       = findSpinType(dicParamValues);
                            response.TotalWin       = double.Parse(dicParamValues["tw"]);
