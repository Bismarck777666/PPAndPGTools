...
{
    class NorthGuardiansFetcher : GameSpinDataFetcher
    {
        public NorthGuardiansFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "s")
                    {
                        ...
                        strResponseHistory.Add(combineResponse(dicParamValues, beforeFreeTotalWin));
                        if(strNextAction == "s" && !dicParamValues.ContainsKey("fs") && double.Parse(dicParamValues["tw"]) == 0.0)
                        {
                            SpinResponse response = new SpinResponse();
                            response.SpinType     = 0;
                            response.TotalWin     = double.Parse(dicParamValues["tw"]);
