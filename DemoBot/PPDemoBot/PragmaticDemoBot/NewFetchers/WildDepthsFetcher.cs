...
{
    class WildDepthsFetcher : GameSpinDataFetcher
    {
        public WildDepthsFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
                base(strProxyInfo, strProxyUserID, strProxyPassword, strClientVersion, realBet, false)
        {
            _isV4 = false;
        }
        ...
        protected override async Task<DoSpinsResults> doSpins(HttpClient httpClient, string strToken)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    SortedDictionary<string, string> dicParams = splitAndRemoveCommonResponse(strResponses[0]);
                    if (!dicParams.ContainsKey("sty"))
                        break;

                    string[] strWilds = dicParams["sty"].Split(new string[] { "~" }, StringSplitOptions.RemoveEmptyEntries);
                    bool mustContinue = false;
                    for(int i = 0; i <  strWilds.Length; i++)
                    {
                        string[] strParts = strWilds[i].Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                        if (strParts[1] != "-1")
                        {
                            mustContinue = true;
                            break;
