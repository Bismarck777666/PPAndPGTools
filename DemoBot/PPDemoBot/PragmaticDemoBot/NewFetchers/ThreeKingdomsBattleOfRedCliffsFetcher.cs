...
{
    class ThreeKingdomsBattleOfRedCliffsFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinCounts = new int[] { 0, 0, 0 };
        public ThreeKingdomsBattleOfRedCliffsFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
                base(strProxyInfo, strProxyUserID, strProxyPassword, strClientVersion, realBet, false)
        {
            _isV4 = false;
        }
        private int selectMinFreeOption()
        {
            int minCount = -1;
            int minIndex = 0;
            for(int i = 0; i < 3; i++)
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "fso")
                    {
                        ...
                        int freeSpinOption = selectMinFreeOption();
