...
{
    class BookOfTutRespinFetcher : GameSpinDataFetcher
    {
        public BookOfTutRespinFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        strResponse = await doBonus(httpClient, strToken, freeSpinType);
                        dicParamValues  = splitAndRemoveCommonResponse(strResponse);
                        strNextAction = dicParamValues["na"];
