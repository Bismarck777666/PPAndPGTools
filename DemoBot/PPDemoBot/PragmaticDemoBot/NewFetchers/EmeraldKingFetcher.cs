...
{
    class EmeraldKingFetcher : GameSpinDataFetcher
    {
        public EmeraldKingFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        private int getMaxID(string[] strParts)
        {
            int maxID = 0;
            for(int i = 0; i < strParts.Length; i++)
            {
                if (maxID < int.Parse(strParts[i]))
                    maxID = int.Parse(strParts[i]);
            }
            return maxID;
        }
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        else if(doBonusID >= 0)
                        {
                            ...
                            if (gParam["rrb"] != null)
                            {
                                string      status   = gParam["rrb"]["status"].ToString();
                                string[]    strParts = status.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                                int maxID = getMaxID(strParts);
                                if (dicParamValues.ContainsKey("rs_p"))
                                    doBonusID = maxID;
                                else
                                    doBonusID = 0;
                            }
