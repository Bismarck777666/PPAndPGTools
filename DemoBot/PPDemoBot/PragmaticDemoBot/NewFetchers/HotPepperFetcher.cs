...
{
    class HotPepperFetcher : GameSpinDataFetcher
    {
        public HotPepperFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "s")
                    {
                        ...
                        strResponseHistory.Add(combineResponse(dicParamValues, beforeFreeTotalWin));
                        if (dicParamValues.ContainsKey("rs_t") && double.Parse(dicParamValues["tw"]) == 0.0 &&
                            dicParamValues["na"] == "s" && !dicParamValues.ContainsKey("fs") && !dicParamValues.ContainsKey("fs_total"))
                        {
                            SpinResponse response   = new SpinResponse();
