...
{
    class BookOfTheFallenFetcher : GameSpinDataFetcher
    {
        private int _lastFreeSpinType = 0;
        public BookOfTheFallenFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        protected KeyValuePair<string, string>[] buildDoPurSpinRequest(string strToken)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
                    ...
                    new KeyValuePair<string, string>("ind",     "8"),
        ...
        protected override async Task<string> doMysteryScatter(HttpClient httpClient, string strToken, int doBonusID)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
            {
            ...
            try
            {
                ...
                do
                {
                    if (strNextAction == "m")
                    {
                        ...
                        int freeSpinOption = 6; // _lastFreeSpinType;
                        _lastFreeSpinType  = (_lastFreeSpinType + 1) % 9;

                        selectedFreeOption  = 200 + freeSpinOption;
                        strResponse         = await doMysteryScatter(httpClient, strToken, freeSpinOption);
