...
{
    class SevenPiggiesFetcher : GameSpinDataFetcher
    {
        public SevenPiggiesFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        string [] strParts = dicParamValues["i_pos"].Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                        strResponse = await doBonus(httpClient, strToken, int.Parse(strParts[doBonusID]));
                        dicParamValues = splitAndRemoveCommonResponse(strResponse);
                        strNextAction = dicParamValues["na"];
                        if (strNextAction != "b")
                            doBonusID = -1;
