...
{
    class SuperXFetcher : GameSpinDataFetcher
    {
        public SuperXFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        protected async Task<string> doBonusFirst(HttpClient httpClient, string strToken, int doBonusID)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
            {
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient, string strToken)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    {
                        if(doBonusID == -1)
                            strResponse = await doBonusFirst(httpClient, strToken, doBonusID);
                        else
