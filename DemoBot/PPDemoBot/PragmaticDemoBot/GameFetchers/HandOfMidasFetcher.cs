...
{
    class HandOfMidasFetcher : GameSpinDataFetcher
    {
        private int _lastPurIndex = 0;
        public HandOfMidasFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
                base(strProxyInfo, strProxyUserID, strProxyPassword, strClientVersion, realBet, hasAnteBet)
        {

        }
        private int nextPurIndex()
        {
            int purIndex = _lastPurIndex;
            _lastPurIndex++;
            if (_lastPurIndex >= 3)
                _lastPurIndex = 0;
            return purIndex;
        }
        protected virtual KeyValuePair<string, string>[] buildDoPurFreeSpinRequest(string strToken, int purIndex)
        {
            ...
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
                    ...
                    new KeyValuePair<string, string>("pur",      purIndex.ToString()),
                    new KeyValuePair<string, string>("mgckey",  strToken),
                    };
            return postValues;
        }
        protected async Task<string> sendPurchaseSpinRequest(HttpClient httpClient, string strToken, int purIndex)
        {
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(buildDoPurFreeSpinRequest(strToken, purIndex));
        ...
        {
            SortedDictionary<string, string> dicParams =  base.removeCommonParts(dicParamValues);
            dicParams.Remove("purtr");
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient, string strToken)
        {
            ...
            try
            {
                int purIndex = 1;
                //nextPurIndex();
                strResponse = await sendPurchaseSpinRequest(httpClient, strToken, purIndex);
                ...
                do
                {
                    ...
                    else if (strNextAction == "c")
                    {
                        ...
                        if (selectedFreeOption == -1)
                        {
                            ...
                            response.TotalWin     = double.Parse(dicParamValues["tw"]);
                            response.Response     = string.Join("\n", strResponseHistory.ToArray());
                            response.FreeSpinType = purIndex;
                            responseList.Add(response);
                        }
                        else
                        {
