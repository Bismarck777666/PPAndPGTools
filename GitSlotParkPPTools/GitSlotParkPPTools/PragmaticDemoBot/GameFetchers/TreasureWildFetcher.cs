...
{
    class TreasureWildFetcher : GameSpinDataFetcher
    {
        protected int _freespinType = 0;
        protected int[] _freespinTypes = new int[] { 9 };

        public TreasureWildFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
                 base(strProxyInfo, strProxyUserID, strProxyPassword, strClientVersion, realBet, hasAnteBet)
        {

        }
        protected  KeyValuePair<string, string>[] buildDoSpinPurRequest(string strToken)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
                    ...
                    new KeyValuePair<string, string>("pur",     _freespinTypes[_freespinType].ToString()),
                    new KeyValuePair<string, string>("mgckey",  strToken),
                    };
            _freespinType = (_freespinType + 1) % _freespinTypes.Length;
            return postValues;
        }
        protected async Task<string> sendPurSpinRequest(HttpClient httpClient, string strToken)
        {
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(buildDoSpinPurRequest(strToken));
