...
{
    class ExtraJuicyMegaFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinTypeCounts = new int[] { 0, 110, 110, 110, 110 };
        public ExtraJuicyMegaFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        protected int selectFreeSpinType()
        {
            int minCount = -1;
            int minIndex = 0;
            for (int i = 0; i < 5; i++)
        ...
        {
            if (dicParams["status"] == "1,0,0,0,0")
                return 0;
            else if (dicParams["status"] == "0,1,0,0,0")
                return 1;
            else if (dicParams["status"] == "0,0,1,0,0")
                return 2;
            else if (dicParams["status"] == "0,0,0,1,0")
                return 3;
            else
                return 4;
        }
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        if (selectedFreeOption == -1)
                        {
                            ...
                            responseList.Add(response);
                            beforeFreeTotalWin = totalWin;
                            selectedFreeOption = selectFreeSpinType();
                        }
