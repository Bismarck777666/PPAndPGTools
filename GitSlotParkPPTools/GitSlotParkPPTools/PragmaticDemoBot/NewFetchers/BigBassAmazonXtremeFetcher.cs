...
{
    internal class BigBassAmazonXtremeFetcher : GameSpinDataFetcher
    {
        public BigBassAmazonXtremeFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet, bool isV4) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        var strStatus = (string)gParam["bg_0"]["status"];
                        var statusArray = strStatus.Split(new string[] {","}, StringSplitOptions.RemoveEmptyEntries);
                        int emptyID     = -1;
                        ...
                        strResponse = await doBonus(httpClient, strToken, emptyID);
                        dicParamValues = splitAndRemoveCommonResponse(strResponse);
                        strNextAction = dicParamValues["na"];
                        if (strNextAction != "b")
                            doBonusID = -1;
