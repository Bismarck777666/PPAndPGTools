...
{
    internal class EmperorCaishenFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinTypeCounts = new int[] { 0, 0, 0, 0, 0 };
        public EmperorCaishenFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        private int selectMinFreeSpinType()
        {
            int minCount = -1;
            int minIndex = 0;
            for(int i = 0; i < _freeSpinTypeCounts.Length; i++)
            {
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient, string strToken)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    {
                        int length = dicParamValues["status"].Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries).Length;
                        if(length == 5)
                        {
