...
{
    class MammothGoldMegaFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinTypeCounts = new int[] { 100, 100, 100, 100, 100, 100, 100, 100, 100, 0 };
        public MammothGoldMegaFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        protected async Task<string> doBonusFirst(HttpClient httpClient, string strToken)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        if (selectedFreeOption == -1)
                        {
                            ...
                            double totalWin = double.Parse(dicParamValues["tw"]);
                            strResponse = await doBonusFirst(httpClient, strToken);
                            dicParamValues      = splitAndRemoveCommonResponse(strResponse);
                            strNextAction       = dicParamValues["na"];
                            strResponseHistory.Add(combineResponse(dicParamValues, beforeFreeTotalWin));
                        ...
                        if(currentFreeSpinType < 9)
                        {
                            ...
                            if (strNextAction == "s" && dicParamValues["trail"] == "fs_n~0" && !dicParamValues.ContainsKey("fsmax"))
                                return responseList;
                        }
                    }
