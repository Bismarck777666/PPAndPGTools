...
{
    public class AztecBlazeFetcher : GameSpinDataFetcher
    {
        public AztecBlazeFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        protected override KeyValuePair<string, string>[] buildDoSpinRequest(string strToken)
        {
            KeyValuePair<string, string>[] postValues = new KeyValuePair<string, string>[]
                        ...
                        new KeyValuePair<string, string>("counter", _counter.ToString()),
                        new KeyValuePair<string, string>("repeat",  "0"),
                        new KeyValuePair<string, string>("bl",      "1"),
                        new KeyValuePair<string, string>("mgckey",  strToken),
                                    };
            return postValues;
        }
        protected int findSpinType(List<string> responseList)
        {
            for (int i = 0; i < responseList.Count; i++)
            {
                SortedDictionary<string, string> dicParamValues = splitAndRemoveCommonResponse(responseList[i]);
                if (dicParamValues.ContainsKey("psym"))
                    return 1;
            }
            return 0;
        }
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "c")
                    {
                        ...
                        {
                            SpinResponse response = new SpinResponse();
                            response.SpinType = findSpinType(strResponseHistory);
                            response.TotalWin = double.Parse(dicParamValues["tw"]);
                            response.Response = string.Join("\n", strResponseHistory.ToArray());
                            responseList.Add(response);
                        }
