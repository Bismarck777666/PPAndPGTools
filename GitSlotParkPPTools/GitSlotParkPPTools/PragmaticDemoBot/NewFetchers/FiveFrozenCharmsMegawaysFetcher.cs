...
{
    class FiveFrozenCharmsMegawaysFetcher : GameSpinDataFetcher
    {
        protected int[] _freeSpinCounts = new int[] { 0, 0, 0, 0, 0, 0, 0 };
        public FiveFrozenCharmsMegawaysFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
                base(strProxyInfo, strProxyUserID, strProxyPassword, strClientVersion, realBet, true)
        {
            _isV4 = false;
        }
        protected int selectFreeSpinType()
        {
            ...
            for(int i = 0; i < 7; i++)
            {
                if (_freeSpinCounts[i] < minCount || minCount == -1)
                {
