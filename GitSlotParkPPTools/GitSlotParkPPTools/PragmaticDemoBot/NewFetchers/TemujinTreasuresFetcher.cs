...
{
    class TemujinTreasuresFetcher : GameSpinDataFetcher
    {
        public TemujinTreasuresFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        {
            List<KeyValuePair<string, string>> postValues = new List<KeyValuePair<string, string>>(new KeyValuePair<string, string>[]
            {
            ...
            if (doBonusID < 0)
                postValues.RemoveAt(postValues.Count - 1);
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(postValues.ToArray());
            HttpResponseMessage message = null;
