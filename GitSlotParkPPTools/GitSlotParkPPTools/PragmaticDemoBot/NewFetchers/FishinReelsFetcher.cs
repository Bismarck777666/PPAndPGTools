...
{
    class FishinReelsFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinTypeCounts = new int[] { 100, 100, 0, 100, 0, 100 };
        public FishinReelsFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        private int getScatterCount(string s)
        {
            string[] strParts = s.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
            int count = 0;
        ...
        private int selectMinFreeSpinType(int freeSpinGroup)
        {
            int id1 = 2 * freeSpinGroup;
            int id2 = 2 * freeSpinGroup + 1;
            if(id2 > 5)
            {

            }
            if (_freeSpinTypeCounts[id1] < _freeSpinTypeCounts[id2])
                return 0;
            else
                return 1;
        }
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient, string strToken)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    {
                        int scatterCount  = getScatterCount(dicParamValues["s"]);
                        ...
                        response.TotalWin       = totalWin;
                        response.RealWin        = totalWin;
                        response.FreeSpinType   = freeSpinGroup;

                        strResponseHistory.Clear();
                        responseList.Add(response);

                        beforeFreeTotalWin = totalWin;
                        int freeSpinOption = selectMinFreeSpinType(freeSpinGroup);
                        selectedFreeOption = 200 + freeSpinGroup * FreeSpinOptionCount + freeSpinOption;
                        strResponse        = await doBonus(httpClient, strToken, freeSpinOption);
                        dicParamValues     = splitAndRemoveCommonResponse(strResponse);
                        strNextAction      = dicParamValues["na"];
                        strResponseHistory.Add(combineResponse(dicParamValues, beforeFreeTotalWin));

                        _freeSpinTypeCounts[freeSpinGroup * FreeSpinOptionCount + freeSpinOption]++;                        
                    }
