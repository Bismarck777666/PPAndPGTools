...
{
    class AladdinsTreasureFetcher : GameSpinDataFetcher
    {
        public AladdinsTreasureFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        strResponse     = await doBonus(httpClient, strToken, doBonusID == 0 ? -1 : 0);
                        dicParamValues  = splitAndRemoveCommonResponse(strResponse);
                        strNextAction   = dicParamValues["na"];
                        if (strNextAction != "b")
                            doBonusID = -1;
