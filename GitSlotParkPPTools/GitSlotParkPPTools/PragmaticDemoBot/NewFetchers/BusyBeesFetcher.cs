...
{
    internal class BusyBeesFetcher : GameSpinDataFetcher
    {
        public BusyBeesFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        {
                            int level = int.Parse(dicParamValues["level"]);                            
                            string[] strParts = dicParamValues["markers"].Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);

                            int firstIndex = 0;
                            for(int i = 0; i < strParts.Length; i++)
                            {
                                if (strParts[i] == (level+1).ToString())
                                {
                                    firstIndex = i;
                                    break;
                                }
                            }
                            doBonusID = firstIndex;
                        }
                        else
                        {
                            doBonusID = -1;
