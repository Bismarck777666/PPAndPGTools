...
{
    class ChristmasCarolMegaFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinCounts = new int[] { 100, 0, 100, 100, 100, 100, 100, 0, 100, 100, 100, 0};
        public ChristmasCarolMegaFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        {
            string[] strOpts  = dicParams["fs_opt"].Split(new string[] { "~" }, StringSplitOptions.RemoveEmptyEntries);
            string[] strParts = strOpts[0].Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);

            if (strParts[0] == "15")
                return 0;
            else if (strParts[0] == "19")
                return 1;
            else if (strParts[0] == "23")
                return 2;
            else
                return 0;
        }
