...
{
    class LadyOfTheMoonFetcher : GameSpinDataFetcher
    {
        public LadyOfTheMoonFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet) :
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        ...
                        strResponse = await doBonus(httpClient, strToken, doBonusID - 1);
                        dicParamValues = splitAndRemoveCommonResponse(strResponse);
                        strNextAction = dicParamValues["na"];
                        if (strNextAction != "b")
                            doBonusID = -1;
