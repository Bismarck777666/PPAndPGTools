...
{
    class TheWildMachineFetcher : GameSpinDataFetcher
    {
        private int[] _freeSpinTypeCounts = new int[2];
        public TheWildMachineFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, string strClientVersion, double realBet, bool hasAnteBet) :
        ...
        {
            if (_freeSpinTypeCounts[0] < _freeSpinTypeCounts[1])
                return 0;
            else
                return 1;
        }
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (strNextAction == "b")
                    {
                        if (doBonusID == -1)
                        {
                            firstBonusWin = double.Parse(dicParamValues["tw"]);
                            doBonusID = 0;
                        }
                        else
                        {
                        ...
                        {
                            double totalWin = firstBonusWin + double.Parse(dicParamValues["rw"]);
                            SpinResponse response = new SpinResponse();
                            response.SpinType = 100;
                            response.Response = string.Join("\n", strResponseHistory.ToArray());
                            response.TotalWin = totalWin;
                            response.RealWin = totalWin;
                            response.FreeSpinType = 0;
