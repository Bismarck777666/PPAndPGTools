...
{
    class QueenOfBountyFetcher : NewPGSpinDataFetcher
    {
        protected int[] _freeSpinCounts = new int[] { 0, 0, 0, 0, 0, 27, 0, 0, 0, 0, 0, 0 };
        public QueenOfBountyFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
        ...
        protected int selectMinFreeSpinType(int freeSpinGroup)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (minCount == -1 || minCount > _freeSpinCounts[i + freeSpinGroup * 3])
                {
                    minCount = _freeSpinCounts[i + freeSpinGroup * 3];
                    minIndex = i;
                }
            }
            return minIndex;
        ...
        protected async Task<string> sendFreeSpinSelectRequest(HttpClient httpClient, int index)
        {
            ...
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("pft", index.ToString()),
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if(nextState == 3)
                    {
                        int freeSpinGroup = response["dt"]["si"]["cbf"]["sc"].ToObject<int>() - 3;
                        int freeSpinType  = selectMinFreeSpinType(freeSpinGroup);
                        ...
                        _freeSpinCounts[freeSpinType + 3 * freeSpinGroup]++;
                        strResponseHistory.Clear();
                        strResponse = await sendFreeSpinSelectRequest(httpClient, freeSpinType);
                        response    = JToken.Parse(strResponse);
                        if (!IsNullOrEmpty(response["err"]))
                            return null;

                        spinType = 200 + freeSpinType + 3 * freeSpinGroup;
