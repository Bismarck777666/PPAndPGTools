...
{
    class MahjongWays2Fetcher : PGSpinDataFetcher
    {
        public MahjongWays2Fetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
        ...
        protected override async Task<string> sendSpinRequest(HttpClient httpClient)
        {
            ...
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("ss", "[]"),
                new KeyValuePair<string, string>("sn", "1"),
