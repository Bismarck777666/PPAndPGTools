...
{
    class CircusDelightFetcher : NewPGSpinDataFetcher
    {
        public CircusDelightFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
            base(strProxyInfo, strProxyUserID, strProxyPassword, gameID, betSize, betLevel)
        {

        }
        
        protected async Task<string> sendFreeSpinMultipleSelect(HttpClient httpClient)
        {
            ...
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("mls",  "0"),
                new KeyValuePair<string, string>("ias", "false"),
        ...
        protected async Task<string> sendFreeSpinCountSelect(HttpClient httpClient)
        {
            string strURL = string.Format("https://api.pg-demo.com/game-api/{0}/v2/Spin?traceId={1}", _strGameSymbol, genRandomId(8));
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("wk", "0_C"),
                new KeyValuePair<string, string>("btt", "2"),
                new KeyValuePair<string, string>("ns",  "0"),
                new KeyValuePair<string, string>("ias", "false"),
                new KeyValuePair<string, string>("atk", _strToken),
                new KeyValuePair<string, string>("pf",  "1"),
            });
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if (nextState == 31)
                    {
                        strResponse = await sendFreeSpinCountSelect(httpClient);
                    ...
                    {
                        strResponse = await sendFreeSpinMultipleSelect(httpClient);
                        response = JToken.Parse(strResponse);
                        if (!IsNullOrEmpty(response["err"]))
                            return null;
