...
{
    internal class TheGreatIcescapeFetcher : PGSpinDataFetcher
    {
        public TheGreatIcescapeFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
            ...
            try
            {
                do
                {
                    ...
                    if (!IsNullOrEmpty(response["dt"]["si"]["fs"]))
                    {
                        ...
                        int remainSpin = response["dt"]["si"]["fs"]["l"]["vb"].ToObject<int>();
                        if (remainSpin > 0)
                        {
                            isBreak = false;
                        }
