...
{
    class FortuneTigerFetcher : PGSpinDataFetcher
    {
        public FortuneTigerFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if (!IsNullOrEmpty(response["dt"]["si"]["irs"]) && (bool) response["dt"]["si"]["irs"] == true)
                        isBreak = false;
