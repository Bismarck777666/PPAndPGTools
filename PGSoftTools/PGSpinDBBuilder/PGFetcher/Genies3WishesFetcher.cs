...
{
    class Genies3WishesFetcher : NewPGSpinDataFetcher
    {
        protected int[] _freeSpinCounts = new int[] { 0, 0, 0};

        public Genies3WishesFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
        ...
        protected async Task<string> sendFreeSpinSelectRequest(HttpClient httpClient, int index)
        {
            ...
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("uft", index.ToString()),
