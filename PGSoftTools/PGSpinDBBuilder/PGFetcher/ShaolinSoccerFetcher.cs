...
{
    class ShaolinSoccerFetcher : NewPGSpinDataFetcher
    {
        protected int[] _freeSpinCounts = new int[] { 0, 0, 0, 0, 0};

        public ShaolinSoccerFetcher(string strProxyInfo, string strProxyUserID, string strProxyPassword, int gameID, float betSize, int betLevel) :
        ...
        protected async Task<string> sendFreeSpinSelectRequest(HttpClient httpClient, int index)
        {
            ...
            FormUrlEncodedContent postContent = new FormUrlEncodedContent(new KeyValuePair<string, string>[]
            {
                ...
                new KeyValuePair<string, string>("ps", (index + 2).ToString()),
        ...
        protected override async Task<List<SpinResponse>> doSpin(HttpClient httpClient)
        {
            ...
            try
            {
                do
                {
                    ...
                    if(nextState == 31)
                    {
                        int minId       = selectMinFreeSpinType();
